...
float NodeHybrid::getHeuristicCost(const Coordinates& node_coords,
                                        ...
                                        const unsigned int&         goal_y)
{
    ...
    if (obstacle_heuristic_lookup_table.size() == size) {
        ...
        std::fill(
            obstacle_heuristic_lookup_table.begin(), obstacle_heuristic_lookup_table.end(), 0.0);
    }
    else {
                ...
                if (existing_cost <= 0.0f) {
                    ...
                    if (existing_cost == 0.0f || -existing_cost > new_cost) {
                        ...
                        obstacle_heuristic_queue.emplace_back(
                            new_cost + distanceHeuristic2D(new_idx, size_x, start_x, start_y),
                            new_idx);
    ...
    NodeVector&                                                                neighbors)
{
    ...
    for (unsigned int i = 0; i != motion_projections.size(); i++) {
        index = NodeHybrid::getIndex(static_cast<unsigned int>(motion_projections[i]._x),
