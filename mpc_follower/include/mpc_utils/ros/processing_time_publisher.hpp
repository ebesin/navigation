...
{
public:
    explicit ProcessingTimePublisher(rclcpp::Node*      node,
                                     const std::string& name = "~/debug/processing_time_ms",
                                     const rclcpp::QoS& qos  = rclcpp::QoS(1))
    {
        pub_processing_time_ = node->create_publisher<diagnostic_msgs::msg::DiagnosticStatus>(name, qos);
    }
...
private:
    ...
    template<class T> std::string to_string_with_precision(const T& value, const int precision)
    {
