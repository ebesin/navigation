 ...
 * @return        {*}
 */
void setTraj(float traj[][4], float first_goal_x, float first_goal_y, float direction, float distance, int size)
{
    std::cout << "direction:" << direction << " cos():" << cos(direction) << "  sin():" << sin(direction) << std::endl;
...
int main()
{
    ...
    std::cout << "航向角: "
              << std::acos(enu.getXEast() / (sqrt(enu.getXEast() * enu.getXEast() + enu.getYNorth() * enu.getYNorth()))) * 180 /
                     M_PI
              << std::endl;
    ...
    for (int i = 0; i < 5; i++) {
        std::cout << "start_x:" << traj[i][0] << "  start_y:" << traj[i][1] << "  goal_x:" << traj[i][2]
                  << " goal_y:" << traj[i][3] << std::endl;
    }
    return 0;
}