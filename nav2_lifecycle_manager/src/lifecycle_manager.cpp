    ...
    , diagnostics_updater_(this)
{
    ...
    get_parameter("attempt_respawn_reconnection",
                  attempt_respawn_reconnection_);
...
void LifecycleManager::createLifecycleServiceClients()
{
    ...
    for (auto& node_name : node_names_) {
        node_map_[node_name] = std::make_shared<LifecycleServiceClient>(
            node_name, shared_from_this());
    }
}
                                          ...
                                          std::uint8_t       transition)
{
    ...
    if (!node_map_[node_name]->change_state(transition) ||
        ...
        RCLCPP_ERROR(get_logger(),
                     "Failed to change state for node: %s",
                     node_name.c_str());
        return false;
    }
...
void LifecycleManager::checkBondConnections()
{
    ...
    for (auto& node_name : node_names_) {
        ...
        if (bond_map_[node_name]->isBroken()) {
            message(std::string("Have not received a heartbeat from " +
                                node_name + "."));

            // if one is down, bring them all down
            RCLCPP_ERROR(get_logger(),
                         "CRITICAL FAILURE: SERVER %s IS DOWN after not "
                         "receiving a heartbeat for %i ms."
            ...
            if (attempt_respawn_reconnection_) {
                bond_respawn_timer_ = this->create_wall_timer(
                    1s,
                    std::bind(&LifecycleManager::checkBondRespawnConnection,
                              this),
                    callback_group_);
            }
            return;
...
void LifecycleManager::checkBondRespawnConnection()
{
    ...
    if (live_servers == max_live_servers) {
        message("Successfully re-established connections from server respawns, "
                "starting back up.");
        bond_respawn_start_time_ = rclcpp::Time(0);
        bond_respawn_timer_.reset();
        startup();
        ...
        message("Failed to re-establish connection from a server crash after "
                "maximum timeout.");
        bond_respawn_start_time_ = rclcpp::Time(0);
        bond_respawn_timer_.reset();
    }
