...
#include "mpc_follower/steering_predictor.hpp"
...
#include "mpc_msgs/msg/float32_multi_array_stamped.hpp"
...
using mpc_msgs::msg::Float32MultiArrayStamped;
...
private:
   ...
   */
    std::pair<bool, MPCData> getData(const MPCTrajectory&  trajectory,
   ...
   */
    std::pair<bool, VectorXd> updateStateForDelayCompensation(const MPCTrajectory& traj,
   ...
   */
    MPCMatrix generateMPCMatrix(const MPCTrajectory& reference_trajectory, const double prediction_dt);
   ...
   */
    std::pair<bool, VectorXd> executeOptimization(const MPCMatrix&     mpc_matrix,
                                                  const VectorXd&      x0,
   ...
   */
    std::pair<bool, MPCTrajectory> resampleMPCTrajectoryByTime(const double         start_time,
                                                               const double         prediction_dt,
   ...
   */
    MPCTrajectory applyVelocityDynamicsFilter(const MPCTrajectory& trajectory, const Odometry& current_kinematics) const;
   ...
   */
    double getPredictionDeltaTime(const double start_time, const MPCTrajectory& input, const Odometry& current_kinematics) const;
   ...
   */
    double calcDesiredSteeringRate(const MPCMatrix& m,
                                   const MatrixXd&  x0,
                                   const MatrixXd&  Uex,
   ...
   */
    ...
    {
        return std::fabs(curvature) < m_param.low_curvature_thresh_curvature ? m_param.low_curvature_weight
   ...
   */
    Trajectory calcPredictedTrajectory(const MPCTrajectory& mpc_resampled_reference_trajectory,
   ...
   */
    Float32MultiArrayStamped generateDiagData(const MPCTrajectory&           reference_trajectory,
    ...
    template<typename... Args> inline bool fail_warn_throttle(Args&&... args) const
    {
    ...
    template<typename... Args> inline void warn_throttle(Args&&... args) const
    {
        RCLCPP_WARN_THROTTLE(m_logger, *m_clock, 3000, args...);
    }
...
public:
   ...
   */
    void setReferenceTrajectory(const Trajectory& trajectory_msg, const TrajectoryFilteringParam& param);
   ...
   */
    inline void setQPSolver(std::shared_ptr<QPSolverInterface> qpsolver_ptr) { m_qpsolver_ptr = qpsolver_ptr; }
   ...
   */
    ...
    {
        m_steering_predictor = std::make_shared<SteeringPredictor>(m_param.steer_tau, m_param.input_delay);
    }
   ...
   */
    inline void initializeLowPassFilters(const double steering_lpf_cutoff_hz, const double error_deriv_lpf_cutoff_hz)
    {
