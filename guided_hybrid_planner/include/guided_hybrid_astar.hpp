...
class GuidedHybridAStar {
 public:
   ...
   * @return        {*}
   */
  GuidedHybridAStar(nav2_costmap_2d::Costmap2D *costmap_ros,
   ...
   * @return        {*}
   */
  GuidedHybridAStar(int width, int height, int dim3,
                    const SearchInfo &serch_info);
   ...
   * @return        {*}
   */
  bool analyticExpansions(const StateNodePtr &current_node,
                          const StateNodePtr &target_node, double &distance);
   ...
   * @return        {*}
   */
  bool getFrontNeighbor(const StateNodePtr &current_node,
                        unsigned int neighbour_index, StateNodePtr &next_node);
   ...
   * @return        {*}
   */
  bool getBackNeighbor(const StateNodePtr &current_node,
                       unsigned int neighbour_index, StateNodePtr &next_node);
  ...
  /**
   * @description  :
   * 将航向角为0时机器人对应的运动原语计算并保存(后续计算任意角度的运动原语时只需做旋转变换)
   * @return        {*}
   */
  void initMotion();
   ...
   * @return        {*}
   */
  double computeObstacleHeuristicCost(const StateNodePtr &current_node,
   ...
   * @return        {*}
   */
  double computeDistanceHeuristicCost(const StateNodePtr &current_node,
                                      const StateNodePtr &target_node);
  double computeEulerDistanceHeuristicCost(const StateNodePtr &current_node,
                                           const StateNodePtr &target_node);
   ...
   * @return        {*}
   */
  double computeHeuristicCost(const StateNodePtr &current_node,
                              const StateNodePtr &target_node);
   ...
   * @return        {*}
   */
  double computeTrajCost(const StateNodePtr &current_node_ptr,
   ...
   * @return        {*}
   */
  void setVisualizationToolsPtr(
      const std::shared_ptr<VisualizationTools> &visualization_tool) {
 ...
 private:
  ...
  struct cmp {
    bool operator()(const std::pair<StateNodePtr, float> left,
