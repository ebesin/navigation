                                                       ...
                                                       const bool                           enable_last_point_compensation)
{
    ...
    for (size_t i = 0; i < path.points.size(); ++i) {
        ...
        cog_pose_with_beta.position    = mpc_utils::getPoint(path.points.at(i));
        cog_pose_with_beta.orientation = mpc_utils::createQuaternionFromYaw(yaw_vec.at(i) - beta);

        const auto rear_pose = mpc_utils::calcOffsetPose(cog_pose_with_beta, -rear_to_cog, 0.0, 0.0);

        // update pose
        mpc_utils::setPose(rear_pose, cog_path.points.at(i));
    }
