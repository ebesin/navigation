                       ...
                       Float32MultiArrayStamped& diagnostic)
{
    ...
    const auto reference_trajectory = applyVelocityDynamicsFilter(m_reference_trajectory, current_kinematics);
    ...
    const auto [success_opt, Uex] = executeOptimization(
        mpc_matrix, x0_delayed, prediction_dt, mpc_resampled_ref_trajectory, current_kinematics.twist.twist.linear.x);
    if (!success_opt) {
    ...
    ctrl_cmd.steering_tire_rotation_rate = static_cast<float>(
        calcDesiredSteeringRate(mpc_matrix, x0_delayed, Uex, u_filtered, current_steer.steering_tire_angle, prediction_dt));
    ...
    predicted_trajectory = calcPredictedTrajectory(mpc_resampled_ref_trajectory, mpc_matrix, x0_delayed, Uex);
...
Trajectory MPC::calcPredictedTrajectory(const MPCTrajectory& mpc_resampled_ref_trajectory,
                                        const MPCMatrix&     mpc_matrix,
                                        const VectorXd&      x0_delayed,
...
Float32MultiArrayStamped MPC::generateDiagData(const MPCTrajectory&           reference_trajectory,
                                               ...
                                               const Odometry&                current_kinematics) const
{
    ...
    const auto append_diag = [&](const auto& val) -> void { diagnostic.data.push_back(static_cast<DiagnosticValueType>(val)); };
...
void MPC::setReferenceTrajectory(const Trajectory& trajectory_msg, const TrajectoryFilteringParam& param)
{
    ...
    const auto is_forward_shift = motion_utils::isDrivingForward(mpc_traj_resampled.toTrajectoryPoints());
    ...
    if (param.enable_path_smoothing && mpc_traj_resampled_size > 2 * param.path_filter_moving_ave_num) {
...
void MPC::resetPrevResult(const SteeringReport& current_steer)
{
    ...
    m_raw_steer_cmd_prev =
        std::clamp(current_steer.steering_tire_angle, static_cast<double>(-steer_lim_f), static_cast<double>(steer_lim_f));
    m_raw_steer_cmd_pprev =
        std::clamp(current_steer.steering_tire_angle, static_cast<double>(-steer_lim_f), static_cast<double>(steer_lim_f));
}
                                      ...
                                      const Odometry&       current_kinematics)
{
    ...
    MPCData data;
    if (!MPCUtils::calcNearestPoseInterp(traj,
                                         current_pose,
                                         &(data.nearest_pose),
                                         &(data.nearest_idx),
                                         &(data.nearest_time),
    ...
    data.yaw_err     = normalizeRadian(tf2::getYaw(current_pose.orientation) - tf2::getYaw(data.nearest_pose.orientation));
    ...
    const double max_prediction_time = m_param.min_prediction_length / static_cast<double>(m_param.prediction_horizon - 1);
...
std::pair<bool, MPCTrajectory> MPC::resampleMPCTrajectoryByTime(const double         ts,
...
std::pair<bool, VectorXd> MPC::updateStateForDelayCompensation(const MPCTrajectory& traj,
...
MPCTrajectory MPC::applyVelocityDynamicsFilter(const MPCTrajectory& input, const Odometry& current_kinematics) const
{
    ...
    const size_t nearest_idx = motion_utils::findFirstNearestIndexWithSoftConstraints(
        autoware_traj.points, current_kinematics.pose.pose, ego_nearest_dist_threshold, ego_nearest_yaw_threshold);

    MPCTrajectory output = input;
    MPCUtils::dynamicSmoothingVelocity(
        nearest_idx, current_kinematics.twist.twist.linear.x, m_param.acceleration_limit, m_param.velocity_time_constant, output);
...
MPCMatrix MPC::generateMPCMatrix(const MPCTrajectory& reference_trajectory, const double prediction_dt)
{
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (i == 0) {
            ...
            for (int j = 0; j < i; ++j) {
                int idx_u_j                                 = j * DIM_U;
                m.Bex.block(idx_x_i, idx_u_j, DIM_X, DIM_U) = Ad * m.Bex.block(idx_x_i_prev, idx_u_j, DIM_X, DIM_U);
            }
...
std::pair<bool, VectorXd> MPC::executeOptimization(
    const MPCMatrix& m, const VectorXd& x0, const double prediction_dt, const MPCTrajectory& traj, const double current_velocity)
{
    VectorXd Uex;
...
void MPC::addSteerWeightR(const double prediction_dt, MatrixXd& R) const
{
    ...
    // add steering acceleration : weight for { (u(i+1) - 2*u(i) + u(i-1)) / dt^2 }^2
    {
        ...
        const Eigen::Matrix3d D = steer_acc_r * (Eigen::Matrix3d() << 1.0, -2.0, 1.0, -2.0, 4.0, -2.0, 1.0, -2.0, 1.0).finished();
        for (int i = 1; i < N - 1; ++i) {
...
void MPC::addSteerWeightF(const double prediction_dt, MatrixXd& f) const
{
    ...
    const double steer_acc_r_cp1 = m_param.nominal_weight.steer_acc / (std::pow(DT, 3) * m_ctrl_period);
    const double steer_acc_r_cp2 = m_param.nominal_weight.steer_acc / (std::pow(DT, 2) * std::pow(m_ctrl_period, 2));
...
double MPC::getPredictionDeltaTime(const double start_time, const MPCTrajectory& input, const Odometry& current_kinematics) const
{
        ...
        for (size_t i = nearest_idx + 1; i < input.relative_time.size(); i++) {
            ...
            if (m_param.min_prediction_length < sum_dist) {
                ...
                const double relative_time_at_i =
                    i == input.relative_time.size() - 1 ? input.relative_time.at(i) - t_ext : input.relative_time.at(i);
                return input.relative_time.at(i - 1) + (relative_time_at_i - input.relative_time.at(i - 1)) * ratio;
            }
        }
    ...
    const double dt = (target_time - start_time) / static_cast<double>(m_param.prediction_horizon - 1);
