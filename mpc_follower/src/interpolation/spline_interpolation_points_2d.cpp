...
std::array<std::vector<double>, 4> getBaseValues(const std::vector<geometry_msgs::msg::Point>& points)
{
    // calculate x, y
    ...
    for (size_t i = 0; i < points.size(); i++) {
        ...
        if (i > 0) {
            ...
            if (std::fabs(current_pos.x - prev_pos.x) < 1e-6 && std::fabs(current_pos.y - prev_pos.y) < 1e-6) {
                continue;
            }
        }
                                                 ...
                                                 const std::vector<double>& query_keys)
{
    ...
    return {
        interpolator_x.getSplineInterpolatedValues(query_keys), interpolator_y.getSplineInterpolatedValues(query_keys), yaw_vec};
}
...
template std::vector<double> splineYawFromPoints(const std::vector<geometry_msgs::msg::Point>& points);

}   // namespace interpolation

geometry_msgs::msg::Pose SplineInterpolationPoints2d::getSplineInterpolatedPose(const size_t idx, const double s) const
{
    geometry_msgs::msg::Pose pose;
    pose.position    = getSplineInterpolatedPoint(idx, s);
    pose.orientation = mpc_utils::createQuaternionFromYaw(getSplineInterpolatedYaw(idx, s));
    return pose;
}

geometry_msgs::msg::Point SplineInterpolationPoints2d::getSplineInterpolatedPoint(const size_t idx, const double s) const
{
...
double SplineInterpolationPoints2d::getSplineInterpolatedYaw(const size_t idx, const double s) const
{
    ...
    const double whole_s = std::clamp(base_s_vec_.at(idx) + s, base_s_vec_.front(), base_s_vec_.back());
...
double SplineInterpolationPoints2d::getSplineInterpolatedCurvature(const size_t idx, const double s) const
{
    if (base_s_vec_.size() <= idx) {
        throw std::out_of_range("idx is out of range.");
    }
    ...
    return (diff_x * quad_diff_y - quad_diff_x * diff_y) / std::pow(std::pow(diff_x, 2) + std::pow(diff_y, 2), 1.5);
}
...
void SplineInterpolationPoints2d::calcSplineCoefficientsInner(const std::vector<geometry_msgs::msg::Point>& points)
{
