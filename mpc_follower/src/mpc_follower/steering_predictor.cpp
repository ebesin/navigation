...
double SteeringPredictor::getSteerCmdSum(const rclcpp::Time& t_start, const rclcpp::Time& t_end, const double time_constant) const
{
    if (m_ctrl_cmd_vec.size() <= 2) {
        return 0.0;
    }
    ...
    while (t_end > rclcpp::Time(m_ctrl_cmd_vec.at(idx).stamp)) {
        ...
        steer_sum +=
            (1 - std::exp(-duration / time_constant)) * static_cast<double>(m_ctrl_cmd_vec.at(idx - 1).steering_tire_angle);
        ++idx;
    ...
    steer_sum += (1 - std::exp(-duration / time_constant)) * static_cast<double>(m_ctrl_cmd_vec.at(idx - 1).steering_tire_angle);
