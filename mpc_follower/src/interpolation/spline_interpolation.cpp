                                  ...
                                  const std::vector<double>& query_keys)
{
    ...
    for (size_t i = 0; i < base_keys.size() - 1; ++i) {
        const double m_val = (base_values.at(i + 1) - base_values.at(i)) / (base_keys.at(i + 1) - base_keys.at(i));
    ...
    for (size_t i = 0; i < base_keys.size(); ++i) {
        ...
        const double denom = std::abs(m_values.at(i + 1) - m_values.at(i)) + std::abs(m_values.at(i - 1) - m_values.at(i - 2));
    ...
    std::vector<double> d;
    for (size_t i = 0; i < base_keys.size() - 1; ++i) {
        a.push_back((s_values.at(i) + s_values.at(i + 1) - 2.0 * m_values.at(i)) /
                    std::pow(base_keys.at(i + 1) - base_keys.at(i), 2));
        b.push_back((3.0 * m_values.at(i) - 2.0 * s_values.at(i) - s_values.at(i + 1)) / (base_keys.at(i + 1) - base_keys.at(i)));
...
void SplineInterpolation::calcSplineCoefficients(const std::vector<double>& base_keys, const std::vector<double>& base_values)
{
    ...
    if (num_base > 2) {
        ...
        for (size_t i = 0; i < num_base - 2; ++i) {
            ...
            tdma_coef.d[i] = 6.0 * (diff_values[i + 1] / diff_keys[i + 1] - diff_values[i] / diff_keys[i]);
        }
    ...
    for (size_t i = 0; i < num_base - 1; ++i) {
        ...
        multi_spline_coef_.c[i] = diff_values[i] / diff_keys[i] - diff_keys[i] * (2 * v[i] + v[i + 1]) / 6.0;
...
std::vector<double> SplineInterpolation::getSplineInterpolatedValues(const std::vector<double>& query_keys) const
{
    // throw exceptions for invalid arguments
...
std::vector<double> SplineInterpolation::getSplineInterpolatedDiffValues(const std::vector<double>& query_keys) const
{
    // throw exceptions for invalid arguments
    const auto validated_query_keys = interpolation_utils::validateKeys(base_keys_, query_keys);
...
std::vector<double> SplineInterpolation::getSplineInterpolatedQuadDiffValues(const std::vector<double>& query_keys) const
{
    // throw exceptions for invalid arguments
    const auto validated_query_keys = interpolation_utils::validateKeys(base_keys_, query_keys);
