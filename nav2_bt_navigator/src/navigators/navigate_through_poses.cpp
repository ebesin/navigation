    ...
    std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
{
    ...
    goals_blackboard_id_ =
        node->get_parameter("goals_blackboard_id").as_string();
    ...
    rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
    ...
    node->get_parameter("default_nav_through_poses_bt_xml",
    ...
    ActionT::Goal::ConstSharedPtr goal)
{
    ...
    if (goal->behavior_tree == bt_action_server_->getCurrentBTFilename() ||
        ...
        RCLCPP_WARN(logger_,
                    "Preemption request was rejected since the requested BT "
                    ...
                    "\nCancel the current goal and send a new action request "
                    "if you want to use a "
                    "different BT XML file. For now, continuing to track the "
                    "last goal until completion.");
        bt_action_server_->terminatePendingGoal();
    }
}

void NavigateThroughPosesNavigator::initializeGoalPoses(
    ActionT::Goal::ConstSharedPtr goal)
{
    if (goal->poses.size() > 0) {
        RCLCPP_INFO(logger_,
                    "Begin navigating from current location through %zu poses "
                    "to (%.2f, %.2f)",
                    goal->poses.size(),
