...
void CarGazeboPlugin::Load(gazebo::physics::ModelPtr model, sdf::ElementPtr sdf)
{
    ...
    joint_state_pub_ = ros_node_->create_publisher<JointState>(
    ...
    tf_broadcaster_ =
        std::make_unique<tf2_ros::TransformBroadcaster>(*ros_node_);

    // fake_car code here
    {
        RCLCPP_DEBUG(ros_node_->get_logger(),
                     "Connected to model %s",
        ...
        // publish
        odo_fl_pub = ros_node_->create_publisher<std_msgs::msg::Int32>(
            "/" + model_->GetName() + "/odo_fl", 10);
        odo_fr_pub = ros_node_->create_publisher<std_msgs::msg::Int32>(
            "/" + model_->GetName() + "/odo_fr", 10);
        ackermann_pub =
            ros_node_
                ->create_publisher<ackermann_msgs::msg::AckermannDriveStamped>(
                    "/" + model_->GetName() + "/cmd_ackermann", 10);

        pose_pub = ros_node_->create_publisher<geometry_msgs::msg::PoseStamped>(
            "/" + model_->GetName() + "/pose", rclcpp::SensorDataQoS());
        ...
        joy_sub = ros_node_->create_subscription<sensor_msgs::msg::Joy>(
            ...
            2,
            std::bind(
                &CarGazeboPlugin::joy_callback, this, std::placeholders::_1));

        ackermann_sub = ros_node_->create_subscription<
            ackermann_msgs::msg::AckermannDriveStamped>(
            "/" + model_->GetName() + "/cmd_ackermann",
            2,
            std::bind(&CarGazeboPlugin::ackermann_callback,
                      this,
                      std::placeholders::_1));
        ...
        cmd_vel_sub = ros_node_->create_subscription<geometry_msgs::msg::Twist>(
            ...
            2,
            std::bind(
                &CarGazeboPlugin::twist_callback, this, std::placeholders::_1));
    }
