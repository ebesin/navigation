...
protected:
   ...
   */
    ...
    float _lookup_table_dim;   //预计算距离启发式函数查找表的大小（单位为栅格数）
    float        _tolerance;
    bool         _downsample_costmap;
    int          _downsampling_factor;   //下采样多少倍
    double       _angle_bin_size;        //其实就是角度采样的分辨率
    unsigned int _angle_quantizations;   //把角度分这么多分
    bool         _allow_unknown;
    int          _max_iterations;
    int          _max_on_approach_iterations;
    SearchInfo   _search_info;
    double       _max_planning_time;
    double _lookup_table_size;   // 预计算距离启发式函数查找的范围（单位应该是m）
