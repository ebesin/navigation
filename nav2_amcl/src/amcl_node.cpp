    ...
    : nav2_util::LifecycleNode("amcl", "", options)
{
    ...
    add_parameter("global_frame_id",
                  ...
                  "The name of the coordinate frame published by the "
                  "localization system");

    add_parameter("lambda_short",
                  rclcpp::ParameterValue(0.1),
                  "Exponential decay parameter for z_short part of model");

    add_parameter("laser_likelihood_max_dist",
                  rclcpp::ParameterValue(2.0),
                  "Maximum distance to do obstacle inflation on map, for use "
                  "in likelihood_field model");

    add_parameter("laser_max_range",
    ...
    add_parameter("recovery_alpha_fast",
                  rclcpp::ParameterValue(0.0),
    ...
    add_parameter("recovery_alpha_slow",
                  rclcpp::ParameterValue(0.0),
    ...
    add_parameter("transform_tolerance",
                  ...
                  "published, to indicate that this transform "
                  "is valid into the future");

    add_parameter("update_min_a",
                  rclcpp::ParameterValue(0.2),
                  "Rotational movement required before performing a filter update");

    add_parameter("update_min_d",
    ...
    add_parameter("always_reset_initial_pose",
                  rclcpp::ParameterValue(false),
    ...
    add_parameter("scan_topic",
                  rclcpp::ParameterValue("scan"),
                  "Topic to subscribe to in order to receive the laser scan "
                  "for localization");

    add_parameter("map_topic",
                  rclcpp::ParameterValue("map"),
                  "Topic to subscribe to in order to receive the map to localize on");

    add_parameter("first_map_only",
                  rclcpp::ParameterValue(false),
...
void AmclNode::initialPoseReceived(geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
{
    ...
    if (msg->header.frame_id == "") {
        ...
        RCLCPP_WARN(get_logger(),
                    "Received initial pose with empty frame_id. You should "
                    "always supply a frame_id.");
        return;
    }
    if (nav2_util::strip_leading_slash(msg->header.frame_id) != global_frame_id_) {
        RCLCPP_WARN(get_logger(),
                    "Ignoring initial pose in frame \"%s\"; initial poses must be in "
                    "the global frame, \"%s\"",
...
void AmclNode::handleInitialPose(geometry_msgs::msg::PoseWithCovarianceStamped& msg)
{
    ...
    try {
        ...
        tx_odom = tf_buffer_->lookupTransform(base_frame_id_,
...
void AmclNode::laserReceived(sensor_msgs::msg::LaserScan::ConstSharedPtr laser_scan)
{
    ...
    pf_vector_t pose;
    if (!getOdomPose(latest_odom_pose_,
                     pose.v[0],
                     pose.v[1],
                     pose.v[2],
...
void AmclNode::publishAmclPose(const sensor_msgs::msg::LaserScan::ConstSharedPtr& laser_scan,
                               ...
                               const int&                                         max_weight_hyp)
{
    ...
    if (!std::isnan(temp)) {
        ...
        RCLCPP_WARN(get_logger(),
                    ...
                    "configuration or faulty sensor measurements! Pose is not "
                    "available!");
    }
...
nav2_amcl::Laser* AmclNode::createLaserObject()
{
    ...
    if (sensor_model_type_ == "likelihood_field_prob") {
        return new nav2_amcl::LikelihoodFieldModelProb(z_hit_,
                                                       z_rand_,
                                                       sigma_hit_,
...
void AmclNode::initParameters()
{
    ...
    if (min_particles_ > max_particles_) {
        RCLCPP_WARN(get_logger(),
                    ...
                    " this isn't allowed so max_particles will be set to "
                    "min_particles.");
            ...
            if (param_name == "alpha1") {
                ...
                if (alpha1_ < 0.0) {
                    RCLCPP_WARN(get_logger(),
                                ...
                                " this isn't allowed, so the alpha1 will be "
                                "set to be zero.");
                    alpha1_ = 0.0;
                }
                reinit_odom = true;
                ...
                if (alpha2_ < 0.0) {
                    RCLCPP_WARN(get_logger(),
                                ...
                                " this isn't allowed, so the alpha2 will be "
                                "set to be zero.");
                    alpha2_ = 0.0;
                }
                reinit_odom = true;
                ...
                if (alpha3_ < 0.0) {
                    RCLCPP_WARN(get_logger(),
                                ...
                                " this isn't allowed, so the alpha3 will be "
                                "set to be zero.");
                    alpha3_ = 0.0;
                }
                reinit_odom = true;
                ...
                if (alpha4_ < 0.0) {
                    RCLCPP_WARN(get_logger(),
                                ...
                                " this isn't allowed, so the alpha4 will be "
                                "set to be zero.");
                    alpha4_ = 0.0;
                }
                reinit_odom = true;
                ...
                if (alpha5_ < 0.0) {
                    RCLCPP_WARN(get_logger(),
                                ...
                                " this isn't allowed, so the alpha5 will be "
                                "set to be zero.");
                    alpha5_ = 0.0;
                }
                reinit_odom = true;
