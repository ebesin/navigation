...
visualization_msgs::msg::MarkerArray VirtualWallMarkerCreator::create_markers(const rclcpp::Time& now)
{
    ...
    for (const auto& virtual_wall : virtual_walls) {
        ...
        case stop: create_fn = motion_utils::createStopVirtualWallMarker; break;
        case slowdown: create_fn = motion_utils::createSlowDownVirtualWallMarker; break;
        case deadline: create_fn = motion_utils::createDeadLineVirtualWallMarker; break;
        }
        auto markers = create_fn(virtual_wall.pose, virtual_wall.text, now, 0, virtual_wall.longitudinal_offset, virtual_wall.ns);
