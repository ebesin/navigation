...
{
    RCLCPP_INFO(
        _logger, "Destroying plugin %s of type SmacPlanner2D", _name.c_str());
}
    ...
    std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
    ...
    node->get_parameter(name + ".cost_travel_multiplier",
    ...
    node->get_parameter(name + ".use_final_approach_orientation",
    ...
    _collision_checker =
        GridCollisionChecker(_costmap, 1 /*for 2D, most be 1*/);
    ...
    _a_star =
        std::make_unique<AStarAlgorithm<Node2D>>(_motion_model, _search_info);
...
{
    RCLCPP_INFO(
        _logger, "Activating plugin %s of type SmacPlanner2D", _name.c_str());
...
{
    RCLCPP_INFO(
        _logger, "Deactivating plugin %s of type SmacPlanner2D", _name.c_str());
...
{
    RCLCPP_INFO(
        _logger, "Cleaning up plugin %s of type SmacPlanner2D", _name.c_str());
    _a_star.reset();
    _smoother.reset();
    if (_costmap_downsampler) {
    ...
    const geometry_msgs::msg::PoseStamped& goal)
{
    ...
    if (mx_start == mx_goal && my_start == my_goal) {
        if (costmap->getCost(mx_start, my_start) ==
    ...
    std::vector<rclcpp::Parameter> parameters)
{
    ...
    if (reinit_a_star || reinit_downsampler) {
        ...
        if (reinit_a_star) {
            _a_star = std::make_unique<AStarAlgorithm<Node2D>>(_motion_model,
                                                               _search_info);
            _a_star->initialize(_allow_unknown,
                                _max_iterations,
                                _max_on_approach_iterations,
...
PLUGINLIB_EXPORT_CLASS(nav2_smac_planner::SmacPlanner2D,
