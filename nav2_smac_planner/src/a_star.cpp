...
template<typename NodeT>
void AStarAlgorithm<NodeT>::setCollisionChecker(
    GridCollisionChecker* collision_checker)
{
    ...
    if (getSizeX() != x_size || getSizeY() != y_size) {
        ...
        NodeT::initMotionModel(
            _motion_model, _x_size, _y_size, _dim3_size, _search_info);
    }
    _expander->setCollisionChecker(collision_checker);
}

template<typename NodeT>
typename AStarAlgorithm<NodeT>::NodePtr
AStarAlgorithm<NodeT>::addToGraph(const unsigned int& index)
{
                                     ...
                                     const unsigned int& dim_3)
{
    ...
    _start->setPose(Coordinates(static_cast<float>(mx),
                                    ...
                                    const unsigned int& dim_3)
{
    ...
    typename NodeT::Coordinates goal_coords(static_cast<float>(mx),
                                            static_cast<float>(my),
                                            static_cast<float>(dim_3));

    if (!_search_info.cache_obstacle_heuristic ||
        goal_coords != _goal_coordinates) {
        if (!_start) {
    ...
    while (iterations < getMaxIterations() && !_queue.empty()) {
        ...
        //这里其实就是计算当前点与目标点间的启发式代价是否在一定范围内，在的话也可以认为到达了目标点。
            ...
            if (approach_iterations >= getOnApproachMaxIterations()) {
                return _graph.at(_best_heuristic_node.second)
                    .backtracePath(path);
            }
        }
...
float AStarAlgorithm<NodeT>::getHeuristicCost(const NodePtr& node)
{
    ...
    float heuristic =
        NodeT::getHeuristicCost(node_coords, _goal_coordinates, _costmap);
