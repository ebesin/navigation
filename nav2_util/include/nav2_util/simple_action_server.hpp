...
public:
        ...
        , spin_thread_(spin_thread)
    {
        ...
        if (spin_thread_) {
            ...
            executor_->add_callback_group(callback_group_,
                                          node_base_interface_);
            executor_thread_ =
                std::make_unique<nav2_util::NodeThread>(executor_);
        }
    }
   ...
   */
        ...
        const std::shared_ptr<rclcpp_action::ServerGoalHandle<ActionT>> handle)
    {
        ...
        if (is_active(current_handle_) || is_running()) {
            debug_msg("An older goal is active, moving the new goal to a "
                      "pending slot.");

            if (is_active(pending_handle_)) {
                debug_msg("The pending slot is occupied."
                          " The previous pending goal will be terminated and "
                          "replaced.");
            ...
            if (is_active(pending_handle_)) {
                ...
                error_msg("Forgot to handle a preemption. Terminating the "
                          "pending goal.");
                terminate(pending_handle_);
    ...
    void deactivate()
    {
        ...
        while (execution_future_.wait_for(milliseconds(100)) !=
            ...
            if (steady_clock::now() - start_time >= server_timeout_) {
                terminate_all();
                completion_callback_();
                throw std::runtime_error("Action callback is still running and "
                                         "missed deadline to stop");
            }
        }
   ...
   */
    ...
    {
        if (!is_active(current_handle_)) {
            error_msg("Trying to publish feedback when the current goal handle "
                      "is not active");
            return;
        }
