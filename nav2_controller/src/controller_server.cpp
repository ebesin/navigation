    ...
    , default_types_{"dwb_core::DWBLocalPlanner"}
{
    ...
    declare_parameter("min_x_velocity_threshold",
                      rclcpp::ParameterValue(0.0001));
    declare_parameter("min_y_velocity_threshold",
                      rclcpp::ParameterValue(0.0001));
    declare_parameter("min_theta_velocity_threshold",
                      rclcpp::ParameterValue(0.0001));

    declare_parameter("speed_limit_topic",
                      rclcpp::ParameterValue("speed_limit"));
...
nav2_util::CallbackReturn ControllerServer::on_configure(
    const rclcpp_lifecycle::State& /*state*/)
{
    ...
    get_parameter("min_theta_velocity_threshold",
    ...
    try {
        ...
        progress_checker_ = progress_checker_loader_.createUniqueInstance(
            progress_checker_type_);
        RCLCPP_INFO(get_logger(),
    ...
    for (size_t i = 0; i != goal_checker_ids_.size(); i++) {
        try {
            ...
            nav2_core::GoalChecker::Ptr goal_checker =
                goal_checker_loader_.createUniqueInstance(
                    goal_checker_types_[i]);
            RCLCPP_INFO(get_logger(),
    ...
    for (size_t i = 0; i != controller_ids_.size(); i++) {
        try {
            ...
            RCLCPP_FATAL(get_logger(),
                         "Failed to create controller. Exception: %s",
                         ex.what());
            return nav2_util::CallbackReturn::FAILURE;
        }
    }
    ...
    speed_limit_sub_ = create_subscription<nav2_msgs::msg::SpeedLimit>(
        speed_limit_topic,
        rclcpp::QoS(10),
        std::bind(&ControllerServer::speedLimitCallback,
                  this,
                  std::placeholders::_1));
...
nav2_util::CallbackReturn ControllerServer::on_activate(
    const rclcpp_lifecycle::State& /*state*/)
{
...
nav2_util::CallbackReturn ControllerServer::on_deactivate(
    const rclcpp_lifecycle::State& /*state*/)
{
...
nav2_util::CallbackReturn ControllerServer::on_cleanup(
    const rclcpp_lifecycle::State& /*state*/)
{
...
nav2_util::CallbackReturn ControllerServer::on_shutdown(
                                         ...
                                         std::string& current_goal_checker)
{
    if (goal_checkers_.find(c_name) == goal_checkers_.end()) {
        if (goal_checkers_.size() == 1 && c_name.empty()) {
            RCLCPP_WARN_ONCE(get_logger(),
                             "No goal checker was specified in parameter "
            ...
            RCLCPP_ERROR(
                ...
                " 'current_goal_checker', which does not exist. Available goal "
                "checkers are: %s.",
                c_name.c_str(),
                goal_checker_ids_concat_.c_str());
            return false;
        ...
        RCLCPP_DEBUG(
            get_logger(), "Selected goal checker: %s.", c_name.c_str());
...
void ControllerServer::computeControl()
{
    ...
    try {
        ...
        std::string gc_name =
            action_server_->get_current_goal()->goal_checker_id;
        std::string current_goal_checker;
    ...
    std::vector<rclcpp::Parameter> parameters)
{
    ...
    for (auto parameter : parameters) {
        ...
        if (!dynamic_params_lock_.try_lock()) {
            ...
            result.reason = "Unable to dynamically change Parameters while the "
                            "controller is currently running";
            return result;
        }
