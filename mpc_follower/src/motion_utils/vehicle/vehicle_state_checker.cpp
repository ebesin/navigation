    ...
    : VehicleStopCheckerBase(node, velocity_buffer_time_sec)
{
    ...
    sub_odom_ = node->create_subscription<Odometry>(
        "/localization/kinematic_state", rclcpp::QoS(1), std::bind(&VehicleStopChecker::onOdom, this, _1));
}
...
VehicleArrivalChecker::VehicleArrivalChecker(rclcpp::Node* node)
    : VehicleStopChecker(node)
{
    using std::placeholders::_1;

    sub_trajectory_ = node->create_subscription<Trajectory>(
        "/planning/scenario_planning/trajectory", rclcpp::QoS(1), std::bind(&VehicleArrivalChecker::onTrajectory, this, _1));
}
...
bool VehicleArrivalChecker::isVehicleStoppedAtStopPoint(const double stop_duration) const
{
    ...
    return std::abs(motion_utils::calcSignedArcLength(trajectory_ptr_->points, p, idx.get())) < th_arrived_distance_m;
}
