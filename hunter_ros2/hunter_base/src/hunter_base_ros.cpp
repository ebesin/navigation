...
HunterBaseRos::HunterBaseRos(std::string node_name)
    ...
    , keep_running_(false)
{
    declare_parameter("port_name", "");   //声明参数

    declare_parameter("odom_frame", "");
    declare_parameter("base_frame", "");
    declare_parameter("odom_topic_name", "");

    declare_parameter("is_hunter_mini", "");
    declare_parameter("is_omni_wheel", "");

    declare_parameter("simulated_robot", false);
    declare_parameter("control_rate", 50);
...
void HunterBaseRos::Run()
{
    ...
    std::unique_ptr<HunterMessenger<HunterRobot>> messenger =
        std::unique_ptr<HunterMessenger<HunterRobot>>(new HunterMessenger<HunterRobot>(robot_, this));
    ...
    RCLCPP_INFO_STREAM(get_logger(), "1111111111111111111111111111111111");
    ...
    RCLCPP_INFO_STREAM(get_logger(), "2222222222222222222222222222222222222");
