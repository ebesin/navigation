...
{
    double lamb = angle2Radian(source.getLatitude());
    double phi  = angle2Radian(source.getLongitude());
    double s    = sin(lamb);
...
ENU wgs84ToEnu(const WGS84& origin, const WGS84& current)
{
    ...
    double phi        = angle2Radian(origin.getLongitude());
    double lamb       = angle2Radian(origin.getLatitude());
    double sin_lambda = sin(lamb);
    double cos_lambda = cos(lamb);
    double sin_phi    = sin(phi);
    double cos_phi    = cos(phi);
    ...
    enu.setZUp(cos_lambda * cos_phi * xd + cos_lambda * sin_phi * yd + sin_lambda * zd);
    return enu;
}
...
{
    double lamb       = angle2Radian(origin.getLatitude());    // 纬度转弧度
    double phi        = angle2Radian(origin.getLongitude());   // 经度转弧度
    ...
    double cos_phi    = cos(phi);
    ECEF   res;
    res.setX(-sin_phi * enu.getXEast() - sin_lambda * cos_phi * enu.getYNorth() + cos_lambda * cos_phi * enu.getZUp() +
             ecef.getX());
    res.setY(cos_phi * enu.getXEast() - sin_lambda * sin_phi * enu.getYNorth() + cos_lambda * sin_phi * enu.getZUp() +
             ecef.getY());
    res.setZ(cos_lambda * enu.getYNorth() + sin_lambda * enu.getZUp() + ecef.getZ());
    return res;
}
...
WGS84 ecefToWgs84(const ECEF& ecef)
{
    ...
    double lat = atan2((z + (d * d) * b * pow(sin(q), 3)), (p - (c * c) * a * pow(cos(q), 3)));
    double N   = a / sqrt(1 - ((c * c) * pow(sin(lat), 2)));
    double alt = (p / cos(lat)) - N;
    WGS84  wgs84(radian2Angle(lon), radian2Angle(lat), alt);
    return wgs84;
}
