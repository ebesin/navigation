...
using mpc_utils::rad2deg;

bool MPC::calculateMPC(const SteeringReport& current_steer,
                       const Odometry& current_kinematics,
                       AckermannLateralCommand& ctrl_cmd, double& v_x,
                       Trajectory& predicted_trajectory,
                       Float32MultiArrayStamped& diagnostic) {
  // since the reference trajectory does not take into account the current
  // velocity of the ego vehicle, it needs to calculate the trajectory velocity
  // considering the longitudinal dynamics.
  ...
  const auto [success_delay, x0_delayed] = updateStateForDelayCompensation(
      reference_trajectory, mpc_data.nearest_time, x0);
  if (!success_delay) {
  ...
  const double prediction_dt = getPredictionDeltaTime(
      mpc_start_time, reference_trajectory, current_kinematics);
  // RCLCPP_INFO(m_logger, "reference_trajectory_size:%d",
  // reference_trajectory.vx.size());
  const auto [success_resample, mpc_resampled_ref_trajectory] =
      resampleMPCTrajectoryByTime(mpc_start_time, prediction_dt,
  ...
  ctrl_cmd.steering_tire_rotation_rate =
      static_cast<float>(calcDesiredSteeringRate(
          mpc_matrix, x0_delayed, Uex, u_filtered,
          current_steer.steering_tire_angle, prediction_dt));
  ...
  predicted_trajectory = calcPredictedTrajectory(mpc_resampled_ref_trajectory,
                                                 mpc_matrix, x0_delayed, Uex);

  // prepare diagnostic message
  diagnostic = generateDiagData(reference_trajectory, mpc_data, mpc_matrix,
                                ctrl_cmd, Uex, current_kinematics);
...
Trajectory MPC::calcPredictedTrajectory(
    ...
    const MPCMatrix& mpc_matrix, const VectorXd& x0_delayed,
    const VectorXd& Uex) const {
  const VectorXd Xex =
      mpc_matrix.Aex * x0_delayed + mpc_matrix.Bex * Uex + mpc_matrix.Wex;
...
Float32MultiArrayStamped MPC::generateDiagData(
    ...
    const MPCMatrix& mpc_matrix, const AckermannLateralCommand& ctrl_cmd,
  ...
  const double nearest_smooth_k =
      reference_trajectory.smooth_k.at(mpc_data.nearest_idx);
  const double wb = m_vehicle_model_ptr->getWheelbase();
  const double current_velocity = current_kinematics.twist.twist.linear.x;
  const double wz_predicted =
      current_velocity * std::tan(mpc_data.predicted_steer) / wb;
  const double wz_measured = current_velocity * std::tan(mpc_data.steer) / wb;
  const double wz_command =
      current_velocity * std::tan(ctrl_cmd.steering_tire_angle) / wb;
  ...
  append_diag(
      ctrl_cmd.steering_tire_angle);   // [0] final steering command (MPC + LPF)
  append_diag(Uex(0));                 // [1] mpc calculation result
  append_diag(mpc_matrix.Uref_ex(0));  // [2] feed-forward steering value
  append_diag(
      std::atan(nearest_smooth_k * wb));  // [3] feed-forward steering value raw
  append_diag(mpc_data.steer);            // [4] current steering angle
  append_diag(mpc_data.lateral_err);      // [5] lateral error
  append_diag(tf2::getYaw(
      current_kinematics.pose.pose.orientation));  // [6] current_pose yaw
  append_diag(
      tf2::getYaw(mpc_data.nearest_pose.orientation));  // [7] nearest_pose yaw
  append_diag(mpc_data.yaw_err);                        // [8] yaw error
  append_diag(reference_trajectory.vx.at(
      mpc_data.nearest_idx));     // [9] reference velocity
  ...
  append_diag(current_velocity *
              nearest_smooth_k);  // [13] angular velocity from path curvature
  append_diag(
      nearest_smooth_k);  // [14] nearest path curvature (used for feed-forward)
...
void MPC::setReferenceTrajectory(const Trajectory& trajectory_msg,
                                 const TrajectoryFilteringParam& param) {
  const auto mpc_traj_raw = MPCUtils::convertToMPCTrajectory(trajectory_msg);

  // resampling
  const auto [success_resample, mpc_traj_resampled] =
      MPCUtils::resampleMPCTrajectoryByDistance(mpc_traj_raw,
                                                param.traj_resample_dist);
  if (!success_resample) {
    warn_throttle(
  ...
  m_is_forward_shift =
      is_forward_shift ? is_forward_shift.get() : m_is_forward_shift;

  // path smoothing
  MPCTrajectory mpc_traj_smoothed =
      mpc_traj_resampled;  // smooth filtered trajectory
  const int mpc_traj_resampled_size =
      static_cast<int>(mpc_traj_resampled.size());
  if (param.enable_path_smoothing &&
      mpc_traj_resampled_size > 2 * param.path_filter_moving_ave_num) {
    using MoveAverageFilter::filt_vector;
    if (!filt_vector(param.path_filter_moving_ave_num, mpc_traj_smoothed.x) ||
   ...
   * Note: The current MPC does not properly take into account the attitude
   * angle at the end of the path. By extending the end of the path in the
   * attitude direction, the MPC can consider the attitude angle well, resulting
   * in improved control performance. If the trajectory is well-defined
   * considering the end point attitude angle, this feature is not necessary.
   */
  if (param.extend_trajectory_for_end_yaw_control) {
    MPCUtils::extendTrajectoryInYawDirection(
        mpc_traj_raw.yaw.back(), param.traj_resample_dist, m_is_forward_shift,
        mpc_traj_smoothed);
  }
  ...
  // calculate curvature
  MPCUtils::calcTrajectoryCurvature(param.curvature_smoothing_num_traj,
                                    param.curvature_smoothing_num_ref_steer,
                                    mpc_traj_smoothed);
  ...
  last_point.relative_time +=
      100.0;  // extra time to prevent mpc calc failure due to short time
...
void MPC::resetPrevResult(const SteeringReport& current_steer) {
  // Consider limit. The prev value larger than limitation brakes the
  // optimization constraint and results in optimization failure.
  const float steer_lim_f = static_cast<float>(m_steer_lim);
  m_raw_steer_cmd_prev = std::clamp(current_steer.steering_tire_angle,
                                    static_cast<double>(-steer_lim_f),
                                    static_cast<double>(steer_lim_f));
  m_raw_steer_cmd_pprev = std::clamp(current_steer.steering_tire_angle,
                                     static_cast<double>(-steer_lim_f),
                                     static_cast<double>(steer_lim_f));
}

std::pair<bool, MPCData> MPC::getData(const MPCTrajectory& traj,
                                      const SteeringReport& current_steer,
                                      const Odometry& current_kinematics) {
  const auto current_pose = current_kinematics.pose.pose;

  MPCData data;
  if (!MPCUtils::calcNearestPoseInterp(
          traj, current_pose, &(data.nearest_pose), &(data.nearest_idx),
          &(data.nearest_time), &(data.ref_vx), ego_nearest_dist_threshold,
  ...
  data.lateral_err =
      MPCUtils::calcLateralError(current_pose, data.nearest_pose);
  data.yaw_err = normalizeRadian(tf2::getYaw(current_pose.orientation) -
  ...
  if (dist_err > m_admissible_position_error) {
    warn_throttle("Too large position error: %fm > %fm", dist_err,
  ...
  if (std::fabs(data.yaw_err) > m_admissible_yaw_error_rad) {
    warn_throttle("Too large yaw error: %f > %f", data.yaw_err,
  ...
  const double max_prediction_time =
      m_param.min_prediction_length /
      static_cast<double>(m_param.prediction_horizon - 1);
  auto end_time = data.nearest_time + m_param.input_delay + m_ctrl_period +
                  max_prediction_time;
...
std::pair<bool, MPCTrajectory> MPC::resampleMPCTrajectoryByTime(
    const double ts, const double prediction_dt,
    const MPCTrajectory& input) const {
  ...
  if (!MPCUtils::linearInterpMPCTrajectory(input.relative_time, input,
                                           mpc_time_v, output)) {
    warn_throttle(
...
VectorXd MPC::getInitialState(const MPCData& data) {
  ...
  const auto& steer =
      m_use_steer_prediction ? data.predicted_steer : data.steer;
  ...
  if (vehicle_model == "kinematics") {
    ...
    RCLCPP_DEBUG(m_logger, "(before lpf) dot_lat_err = %f, dot_yaw_err = %f",
                 dlat, dyaw);
    RCLCPP_DEBUG(m_logger, "(after lpf) dot_lat_err = %f, dot_yaw_err = %f",
                 dlat, dyaw);
  } else {
...
std::pair<bool, VectorXd> MPC::updateStateForDelayCompensation(
    const MPCTrajectory& traj, const double& start_time,
    const VectorXd& x0_orig) {
  const int DIM_X = m_vehicle_model_ptr->getDimX();
  ...
  for (size_t i = 0; i < m_input_buffer.size(); ++i) {
    ...
    try {
      ...
      RCLCPP_ERROR(m_logger,
                   "mpc resample failed at delay compensation, stop mpc: %s",
                   e.what());
      return {false, {}};
    }
...
MPCTrajectory MPC::applyVelocityDynamicsFilter(
  ...
  const size_t nearest_idx =
      motion_utils::findFirstNearestIndexWithSoftConstraints(
          autoware_traj.points, current_kinematics.pose.pose,
          ego_nearest_dist_threshold, ego_nearest_yaw_threshold);

  MPCTrajectory output = input;
  MPCUtils::dynamicSmoothingVelocity(
      nearest_idx, current_kinematics.twist.twist.linear.x,
      m_param.acceleration_limit, m_param.velocity_time_constant, output);
 ...
 * cost function: J = Xex' * Qex * Xex + (Uex - Uref)' * R1ex * (Uex - Uref_ex)
 * + Uex' * R2ex * Uex Qex = diag([Q,Q,...]), R1ex = diag([R,R,...])
 */
MPCMatrix MPC::generateMPCMatrix(const MPCTrajectory& reference_trajectory,
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (i == 0) {
      ...
      m.Aex.block(idx_x_i, 0, DIM_X, DIM_X) =
          Ad * m.Aex.block(idx_x_i_prev, 0, DIM_X, DIM_X);
      for (int j = 0; j < i; ++j) {
        int idx_u_j = j * DIM_U;
        m.Bex.block(idx_x_i, idx_u_j, DIM_X, DIM_U) =
            Ad * m.Bex.block(idx_x_i_prev, idx_u_j, DIM_X, DIM_U);
      }
      m.Wex.block(idx_x_i, 0, DIM_X, 1) =
          Ad * m.Wex.block(idx_x_i_prev, 0, DIM_X, 1) + Wd;
    }
    ...
    if (std::fabs(Uref(0, 0)) < mpc_utils::deg2rad(m_param.zero_ff_steer_deg)) {
 ...
 * (2)lbA < Au < ubA ...
 * [prev_u0 - au_lim*ctp] < [   u0  ] < [prev_u0 + au_lim*ctp] (*ctp ...
 * ctrl_period) [    -au_lim * dt    ] < [u1 - u0] < [     au_lim * dt    ] [
 * -au_lim * dt    ] < [u2 - u1] < [     au_lim * dt    ]
 *                            ~~~
 * [    -au_lim * dt    ] < [uN-uN-1] < [     au_lim * dt    ] (*N... DIM_U)
 */
std::pair<bool, VectorXd> MPC::executeOptimization(
    const MPCMatrix& m, const VectorXd& x0, const double prediction_dt,
  ...
  const int DIM_U_N =
      m_param.prediction_horizon * m_vehicle_model_ptr->getDimU();

  // cost function: 1/2 * Uex' * H * Uex + f' * Uex,  H = B' * C' * Q * C * B +
  // R
  const MatrixXd CB = m.Cex * m.Bex;
  const MatrixXd QCB = m.Qex * CB;
  // MatrixXd H = CB.transpose() * QCB + m.R1ex + m.R2ex; // This calculation is
  // heavy. looking for a good way.  //NOLINT
  ...
  // std::cout << "x0:" << std::endl;
  // std::cout << x0 << std::endl;
  MatrixXd f = (m.Cex * (m.Aex * x0 + m.Wex)).transpose() * QCB -
  ...
  const auto get_adaptive_steer_rate_lim = [&](const double curvature,
    ...
    double steer_rate_lim_by_curvature =
        m_steer_rate_lim_map_by_curvature.back().second;
    ...
    double steer_rate_lim_by_velocity =
        m_steer_rate_lim_map_by_velocity.back().second;
  ...
  VectorXd lb =
      VectorXd::Constant(DIM_U_N, -m_steer_lim);           // min steering angle
  ...
  //     get_adaptive_steer_rate_lim(traj.smooth_k.at(i), traj.vx.at(i)); const
  //     double adaptive_delta_steer_lim = adaptive_steer_rate_lim *
  //     prediction_dt; lbA(i)                                =
  //     -adaptive_delta_steer_lim; ubA(i)                                =
  //     adaptive_delta_steer_lim;
  // }
  // const double adaptive_steer_rate_lim =
  // get_adaptive_steer_rate_lim(traj.smooth_k.at(0), traj.vx.at(0)); lbA(0, 0)
  // = m_raw_steer_cmd_prev - adaptive_steer_rate_lim * m_ctrl_period; ubA(0, 0)
  // = m_raw_steer_cmd_prev + adaptive_steer_rate_lim * m_ctrl_period; std::cout
  // << H << std::endl;
...

  {
    auto t =
        std::chrono::duration_cast<std::chrono::milliseconds>(t_end - t_start)
            .count();
    // auto t = std::chrono::duration_cast<std::chrono::milliseconds>(t_end -
    // t_start).count();
...
void MPC::addSteerWeightR(const double prediction_dt, MatrixXd& R) const {
  ...
  // add steering rate : weight for (u(i) - u(i-1) / dt )^2
  {
    ...
    const Eigen::Matrix2d D =
        steer_rate_r * (Eigen::Matrix2d() << 1.0, -1.0, -1.0, 1.0).finished();
    for (int i = 0; i < N - 1; ++i) {
      R.block(i, i, 2, 2) += D;
    }
    if (N > 1) {
      // steer rate i = 0
      R(0, 0) +=
          m_param.nominal_weight.steer_rate / (m_ctrl_period * m_ctrl_period);
    }
  }

  // add steering acceleration : weight for { (u(i+1) - 2*u(i) + u(i-1)) / dt^2
  // }^2
  {
    ...
    const double steer_acc_r_cp2 =
        w / (std::pow(DT, 2) * std::pow(m_ctrl_period, 2));
    const double steer_acc_r_cp4 = w / std::pow(m_ctrl_period, 4);
    const Eigen::Matrix3d D =
        steer_acc_r *
        (Eigen::Matrix3d() << 1.0, -2.0, 1.0, -2.0, 4.0, -2.0, 1.0, -2.0, 1.0)
            .finished();
    for (int i = 1; i < N - 1; ++i) {
    ...
    if (N > 1) {
      // steer acc i = 1
      R(0, 0) +=
          steer_acc_r * 1.0 + steer_acc_r_cp2 * 1.0 + steer_acc_r_cp1 * 2.0;
...
void MPC::addSteerWeightF(const double prediction_dt, MatrixXd& f) const {
  ...
  const double steer_acc_r_cp2 = m_param.nominal_weight.steer_acc /
                                 (std::pow(DT, 2) * std::pow(m_ctrl_period, 2));
  const double steer_acc_r_cp4 =
      m_param.nominal_weight.steer_acc / std::pow(m_ctrl_period, 4);

  // steer acc  i = 0
  f(0, 0) += ((-2.0 * m_raw_steer_cmd_prev + m_raw_steer_cmd_pprev) *
              steer_acc_r_cp4) *
             0.5;

  // steer acc for i = 1
  f(0, 0) +=
      (-2.0 * m_raw_steer_cmd_prev * (steer_acc_r_cp1 + steer_acc_r_cp2)) * 0.5;
  f(0, 1) += (2.0 * m_raw_steer_cmd_prev * steer_acc_r_cp1) * 0.5;
}

double MPC::getPredictionDeltaTime(const double start_time,
  ...
  const double target_time = [&]() {
    const double t_ext = 100.0;  // extra time to prevent mpc calculation
                                 // failure due to short time
    for (size_t i = nearest_idx + 1; i < input.relative_time.size(); i++) {
      ...
      if (m_param.min_prediction_length < sum_dist) {
        ...
        const double ratio =
            (m_param.min_prediction_length - prev_sum_dist) / segment_dist;
        const double relative_time_at_i =
            i == input.relative_time.size() - 1
  ...
  const double dt = (target_time - start_time) /
                    static_cast<double>(m_param.prediction_horizon - 1);

  return std::max(dt, m_param.prediction_dt);
}

double MPC::calcDesiredSteeringRate(const MPCMatrix& mpc_matrix,
                                    const MatrixXd& x0, const MatrixXd& Uex,
                                    const double u_filtered,
                                    const float current_steer,
                                    const double predict_dt) const {
...
bool MPC::isValid(const MPCMatrix& m) const {
  if (m.Aex.array().isNaN().any() || m.Bex.array().isNaN().any() ||
      m.Cex.array().isNaN().any() || m.Wex.array().isNaN().any() ||
      m.Qex.array().isNaN().any() || m.R1ex.array().isNaN().any() ||
      m.R2ex.array().isNaN().any() || m.Uref_ex.array().isNaN().any()) {
    return false;
  }

  if (m.Aex.array().isInf().any() || m.Bex.array().isInf().any() ||
      m.Cex.array().isInf().any() || m.Wex.array().isInf().any() ||
      m.Qex.array().isInf().any() || m.R1ex.array().isInf().any() ||
