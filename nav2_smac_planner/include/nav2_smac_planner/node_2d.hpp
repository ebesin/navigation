...
public:
   ...
   * @brief 获取当前点到周围点的代价，周围点就是相邻的8个点
   ...
   * 初始化cost_travel_multiplier以及_neighbors_grid_offsets，把周围八个节点相对当前节点的偏移量存入_neighbors_grid_offsets中。
   ...
   * 根据_neighbors_grid_offsets内保存的偏移量获取邻居节点，并根据一定优先级存入neighbors.
