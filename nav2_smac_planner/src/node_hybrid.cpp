...
namespace nav2_smac_planner
{
    ...
    void HybridMotionTable::initDubin(unsigned int &size_x_in,
                                      ...
                                      SearchInfo &search_info)
    {
        ...
        else
        {
            ...
            // 这里应该只是想取一个整数，这样采样角度就是bin_size的整数倍，而不是小数倍。
                                           ...
                                           SearchInfo &search_info)
    {
        ...
        angle = increments * bin_size;
        // increments就是每次采样时角度的最小量（多少个bin_size，因为车的当前航向角是用bin_size为基本单位大致表示的）
        ...
        projections.emplace_back(
            delta_x, -delta_y, -increments);                           // Forward + Right
        projections.emplace_back(-hypotf(delta_x, delta_y), 0.0, 0.0); // Backward
        projections.emplace_back(
            -delta_x, delta_y, -increments); // Backward + Left
        projections.emplace_back(
            -delta_x, -delta_y, increments); // Backward + Right
        ...
        delta_ys.resize(projections.size());
        trig_values.resize(num_angle_quantization);
        // delta_xs和delta_ys中应该记录了projections中6种运动在theta角度航向角改变gngle°后x与y的偏移量
        for (unsigned int i = 0; i != projections.size(); i++)
        {
            delta_xs[i].resize(num_angle_quantization);
            delta_ys[i].resize(num_angle_quantization);
            ...
            for (unsigned int j = 0; j != num_angle_quantization; j++)
            {
                ...
                // 在bin_size * j这个角度航向角旋转angle度的x和y的偏移量
                // （在前面计算出了在0度时旋转angle度所得到的偏移量，这个偏移量实际上就是这段圆弧的终点坐标[x,y]，
                // 要得到在theta角时旋转同样angle角得到的偏移量的话，只需要将[x,y]做旋转变换，旋转角度为theta）
                delta_xs[i][j] =
                    projections[i]._x * cos_theta - projections[i]._y * sin_theta;
                delta_ys[i][j] =
                    projections[i]._x * sin_theta + projections[i]._y * cos_theta;
    ...
    NodeHybrid::NodeHybrid(const unsigned int index)
        : parent(nullptr), pose(0.0f, 0.0f, 0.0f), _cell_cost(std::numeric_limits<float>::quiet_NaN()), _accumulated_cost(std::numeric_limits<float>::max()), _index(index), _was_visited(false), _motion_primitive_index(std::numeric_limits<unsigned int>::max())
    {
    }
    ...
    {
        if (collision_checker->inCollision(this->pose.x,
                                           this->pose.y,
    ...
    float NodeHybrid::getTraversalCost(const NodePtr &child)
    {
        ...
        if (child->getMotionPrimitiveIndex() == 0 ||
            child->getMotionPrimitiveIndex() == 3)
        {
                                     ...
                                     SearchInfo &search_info)
    {
        ...
        case MotionModel::DUBIN:
            motion_table.initDubin(
                size_x, size_y, num_angle_quantization, search_info);
            break;
        case MotionModel::REEDS_SHEPP:
            motion_table.initReedsShepp(
                size_x, size_y, num_angle_quantization, search_info);
            break;
        default:
    ...
    void NodeHybrid::resetObstacleHeuristic(nav2_costmap_2d::Costmap2D *costmap,
                                            ...
                                            const unsigned int &goal_y)
    {
        ...
        downsampler.on_configure(
            ptr, "fake_frame", "fake_topic", costmap, 2.0, true);
        ...
        if (obstacle_heuristic_lookup_table.size() == size)
        {
            ...
            std::fill(obstacle_heuristic_lookup_table.begin(),
                      obstacle_heuristic_lookup_table.end(),
                      0.0);
        }
        else
        {
        ...
        obstacle_heuristic_queue.reserve(sampled_costmap->getSizeInCellsX() *
    ...
    // 就是算A*代价，只不过算的过程中保存了一些中间数据，比如每次找到的最小代价值、计算过程中产生的节点到终点的临时代价值等，方便下一次计算使用。
    float NodeHybrid::getObstacleHeuristic(const Coordinates &node_coords,
                                           const Coordinates &goal_coords,
                                           const double &cost_penalty)
    {
        ...
        const float &requested_node_cost =
            obstacle_heuristic_lookup_table[start_index];
        ...
        // 这里其实就是用A*算法找到从终点到起点的代价值，obstacle_heuristic_queue保存的就是当前点到起点的g_cost+h_cost
        // obstacle_heuristic_lookup_table里面保存的是栅格地图中的节点到终点的代价值
        // 基本思路是从终点开始，以起点为目标点做A*搜索，搜索过程中产生的终点到过程中节点的代价保存到obstacle_heuristic_lookup_table中，以便后续直接查找。
        // 直到搜索到目标点，终点到目标点的最终代价值作为函数返回值。

        // 初始化obstacle_heuristic_queue里面的代价值，代价值为每个点到终点的cost+到起点的距离代价（这里obstacle_heuristic_lookup_table[n.second]为负，表示在open_set里面）
        ...
        while (!obstacle_heuristic_queue.empty())
        {
            ...
            c_cost = -c_cost;
            obstacle_heuristic_lookup_table[idx] =
            ...
            {
                new_idx = static_cast<unsigned int>(static_cast<int>(idx) +
                                                    neighborhood[i]);
                ...
                if (new_idx < size_x * size_y)
                {
                    ...
                    if (existing_cost <= 0.0f)
                    {
                        ...
                        if (existing_cost == 0.0f || -existing_cost > new_cost)
                        {
                            ...
                            obstacle_heuristic_queue.emplace_back(
                                new_cost + distanceHeuristic2D(
                                               new_idx, size_x, start_x, start_y),
                                new_idx);
    ...
    // 如果在距离终点一定范围内，则返回该点到终点的RS曲线长度，否则返回0
    float NodeHybrid::getDistanceHeuristic(const Coordinates &node_coords,
                                           const Coordinates &goal_coords,
                                           const float &obstacle_heuristic)
    {
        ...
        // 因为dist_heuristic_lookup_table中保存了以（0，0，0）为终点坐标，所有x∈[-size_lookup/2，size_lookup/2],y∈[0，size_lookup/2]内的点
        // 为起点坐标的距离启发式代价，所以下面是把起点和终点之间的相对位置通过旋转等变换到了以终点为原点的坐标系下，这样就可以直接查找这个表了。
        ...
        // 将坐标旋转到终点坐标对应的坐标系下
        Coordinates node_coords_relative(round(dx * cos_th - dy * sin_th),
        ...
        // 如果在一定范围内（这个范围就是以终点坐标向量为坐标系，看是否在以floored_size为半径的矩形框内）
        if (abs(node_coords_relative.x) < floored_size &&
            mirrored_relative_y < floored_size)
        {
            // Need to mirror angle if Y coordinate was mirrored
            int theta_pos;
            // 由于只保存了第一二象限的，所以要变换到一二象限，要把坐标点变换到以x轴为对称轴的对应位置上
            if (node_coords_relative.y < 0.0)
            {
                theta_pos = motion_table.num_angle_quantization -
                            node_coords_relative.theta;
            }
            else
            {
        ...
        else if (obstacle_heuristic <= 0.0)
        {
            ...
            static ompl::base::ScopedState<> from(motion_table.state_space),
                to(motion_table.state_space);
            to[0] = goal_coords.x;
            to[1] = goal_coords.y;
    ...
    void NodeHybrid::precomputeDistanceHeuristic(const float &lookup_table_dim,
                                                 ...
                                                 const SearchInfo &search_info)
    {
        ...
        if (motion_model == MotionModel::DUBIN)
        {
            motion_table.state_space =
                std::make_unique<ompl::base::DubinsStateSpace>(
                    search_info.minimum_turning_radius);
        }
        else if (motion_model == MotionModel::REEDS_SHEPP)
        {
            motion_table.state_space =
                std::make_unique<ompl::base::ReedsSheppStateSpace>(
                    search_info.minimum_turning_radius);
        }
        else
        {
        ...
        ompl::base::ScopedState<> from(motion_table.state_space),
            to(motion_table.state_space);
        to[0] = 0.0;
        to[1] = 0.0;
        to[2] = 0.0;
        ...
        dist_heuristic_lookup_table.resize(size_lookup * ceil(size_lookup / 2.0) *
                                           dim_3_size_int);
        // 只计算x∈[-size_lookup / 2.0]和y∈[0,size_lookup / 2.0]区间内的数据，因为对称性，另一半可以对称得到
        for (float x = ceil(-size_lookup / 2.0); x <= floor(size_lookup / 2.0);
             x += 1.0)
        {
            for (float y = 0.0; y <= floor(size_lookup / 2.0); y += 1.0)
            {
                for (int heading = 0; heading != dim_3_size_int; heading++)
                {
                    ...
                    motion_heuristic =
                        motion_table.state_space->distance(from(), to());
    ...
    void NodeHybrid::getNeighbors(
        std::function<bool(const unsigned int &, nav2_smac_planner::NodeHybrid *&)> &
            NeighborGetter,
        GridCollisionChecker *collision_checker,
    ...
    bool NodeHybrid::backtracePath(CoordinateVector &path)
    {
        ...
        while (current_node->parent)
        {
            ...
            path.back().theta =
                NodeHybrid::motion_table.getAngleFromBin(path.back().theta);
