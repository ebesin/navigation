...
void LatticeMotionTable::initMotionModel(unsigned int& size_x_in,
                                         SearchInfo&   search_info)
{
    ...
    for (unsigned int i = 0; i < lattice_metadata.heading_angles.size(); ++i) {
        trig_values.emplace_back(cos(lattice_metadata.heading_angles[i]),
    ...
    const NodeLattice* node)
{
    ...
    if (allow_reverse_expansion) {
        ...
        double reserve_heading =
            node->pose.theta - (num_angle_quantization / 2);
        ...
        MotionPrimitives& prims_at_reverse_heading =
            motion_primitives[reserve_heading];
...
LatticeMetadata LatticeMotionTable::getLatticeMetadata(
...
unsigned int LatticeMotionTable::getClosestAngularBin(const double& theta)
{
    ...
    float        dist        = 0.0;
    for (unsigned int i = 0; i != lattice_metadata.heading_angles.size(); i++) {
        dist = fabs(angles::shortest_angular_distance(
            theta, lattice_metadata.heading_angles[i]));
        if (dist < min_dist) {
            min_dist    = dist;
            closest_idx = i;
        }
...
bool NodeLattice::isNodeValid(const bool&           traverse_unknown,
                              ...
                              bool                  is_backwards)
{
    ...
    static const double bin_size =
        2.0 * M_PI / collision_checker->getPrecomputedAngles().size();
    const double& angle =
        motion_table.getAngleFromBin(this->pose.theta) / bin_size;
    if (collision_checker->inCollision(this->pose.x,
                                       this->pose.y,
                                       angle /*bin in collision checker*/,
                                       traverse_unknown)) {
        return false;
    }
    ...
    if (motion_primitive) {
        ...
        const float& grid_resolution =
            motion_table.lattice_metadata.grid_resolution;
        const float& resolution_diag_sq =
            2.0 * grid_resolution * grid_resolution;
        ...
        initial_pose._x = this->pose.x -
                          (motion_primitive->poses.back()._x / grid_resolution);
        initial_pose._y = this->pose.y -
                          (motion_primitive->poses.back()._y / grid_resolution);
        initial_pose._theta =
            motion_table.getAngleFromBin(motion_primitive->start_angle);

        for (auto it = motion_primitive->poses.begin();
             it != motion_primitive->poses.end();
             ++it) {
            ...
            if (pose_dist._x * pose_dist._x + pose_dist._y * pose_dist._y >
                resolution_diag_sq) {
                last_pose = *it;
                ...
                if (collision_checker->inCollision(
                        ...
                        prim_pose._y,
                        prim_pose._theta /
                            bin_size /*bin in collision checker*/,
                        traverse_unknown)) {
                    return false;
                }
                max_cell_cost =
                    std::max(max_cell_cost, collision_checker->getCost());
            }
        }
    }
...
float NodeLattice::getTraversalCost(const NodePtr& child)
{
    ...
    const float      prim_length     = transition_prim->trajectory_length /
                              motion_table.lattice_metadata.grid_resolution;
    if (prim == nullptr) {
        return prim_length;
    }
...
float NodeLattice::getHeuristicCost(const Coordinates& node_coords,
...
void NodeLattice::initMotionModel(const MotionModel& motion_model,
                                  unsigned int&      size_x,
                                  unsigned int& /*size_y*/,
...
float NodeLattice::getDistanceHeuristic(const Coordinates& node_coords,
                                        const Coordinates& goal_coords,
...
void NodeLattice::precomputeDistanceHeuristic(const float& lookup_table_dim,
...
void NodeLattice::getNeighbors(
    std::function<bool(const unsigned int&, nav2_smac_planner::NodeLattice*&)>&
                          NeighborGetter,
    GridCollisionChecker* collision_checker,
    ...
    NodeVector&           neighbors)
{
    ...
    MotionPrimitivePtrs motion_primitives =
        motion_table.getMotionPrimitives(this);
    const float& grid_resolution =
        motion_table.lattice_metadata.grid_resolution;

    unsigned int direction_change_idx = 1e9;
    for (unsigned int i = 0; i != motion_primitives.size(); i++) {
        if (motion_primitives[0]->start_angle !=
            motion_primitives[i]->start_angle) {
    ...
    for (unsigned int i = 0; i != motion_primitives.size(); i++) {
        ...
        motion_projection.theta =
            motion_primitives[i]->end_angle /*this is the ending angular bin*/;
        ...
        if (NeighborGetter(index, neighbor) && !neighbor->wasVisited()) {
            ...
            neighbor->setPose(
                Coordinates(motion_projection.x, motion_projection.y, angle));
...
void NodeLattice::addNodeToPath(NodeLattice::NodePtr           current_node,
                                NodeLattice::CoordinateVector& path)
{
    Coordinates      initial_pose, prim_pose;
    MotionPrimitive* prim = nullptr;
    const float&     grid_resolution =
        NodeLattice::motion_table.lattice_metadata.grid_resolution;
    const float pi_2 = 2.0 * M_PI;
    ...
    if (prim) {
        initial_pose.x =
            current_node->pose.x - (prim->poses.back()._x / grid_resolution);
        initial_pose.y =
            current_node->pose.y - (prim->poses.back()._y / grid_resolution);
        initial_pose.theta =
            NodeLattice::motion_table.getAngleFromBin(prim->start_angle);
        ...
        path.back().theta =
            NodeLattice::motion_table.getAngleFromBin(path.back().theta);
    }
}
