...
AnalyticExpansion<NodeT>::AnalyticExpansion(const MotionModel& motion_model,
...
template<typename NodeT>
typename AnalyticExpansion<NodeT>::NodePtr
AnalyticExpansion<NodeT>::tryAnalyticExpansion(const NodePtr&    current_node,
                                               ...
                                               int& closest_distance)
{
    ...
    if (_motion_model == MotionModel::DUBIN ||
        ...
        const Coordinates node_coords = NodeT::getCoords(
            current_node->getIndex(), costmap->getSizeInCellsX(), _dim_3_size);
        closest_distance =
            std::min(closest_distance,
                     static_cast<int>(NodeT::getHeuristicCost(
                         node_coords, goal_node->pose, costmap)));
        ...
        int desired_iterations = std::max(
            static_cast<int>(closest_distance /
        ...
        if (analytic_iterations <= 0) {
            ...
            AnalyticExpansionNodes analytic_nodes =
                getAnalyticPath(current_node, goal_node, getter);
                ...
                for (int i = 0; i < 8; i++) {
                    ...
                    if (test_node->parent && test_node->parent->parent &&
                        ...
                        refined_analytic_nodes =
                            getAnalyticPath(test_node, goal_node, getter);
...
AnalyticExpansion<NodeT>::getAnalyticPath(const NodePtr&    node,
                                          const NodePtr&    goal,
                                          const NodeGetter& node_getter)
{
    ...
    // num_intervals == 0
    possible_nodes.reserve(
        num_intervals);   // We won't store this node or the goal
    ...
    for (float i = 1; i < num_intervals; i++) {
        node->motion_table.state_space->interpolate(
            from(), to(), i / num_intervals, s());
        reals = s.reals();
        ...
        index = NodeT::getIndex(static_cast<unsigned int>(reals[0]),
        ...
        if (node_getter(index, next)) {
            ...
            proposed_coordinates            = {static_cast<float>(reals[0]),
                                               static_cast<float>(reals[1]),
                                               angle};
            ...
            if (next->isNodeValid(_traverse_unknown, _collision_checker) &&
                ...
                possible_nodes.emplace_back(
                    next, initial_node_coords, proposed_coordinates);
                prev = next;
            }
            else {
                // Abort
...
template<typename NodeT>
typename AnalyticExpansion<NodeT>::NodePtr
AnalyticExpansion<NodeT>::setAnalyticPath(
    const NodePtr&                node,
    const NodePtr&                goal_node,
...
template<> void AnalyticExpansion<NodeLattice>::cleanNode(const NodePtr& node)
{
...
AnalyticExpansion<Node2D>::getAnalyticPath(const NodePtr&    node,
                                           const NodePtr&    goal,
                                           const NodeGetter& node_getter)
{
    return AnalyticExpansionNodes();
...
template<>
typename AnalyticExpansion<Node2D>::NodePtr
AnalyticExpansion<Node2D>::setAnalyticPath(
    const NodePtr&                node,
    const NodePtr&                goal_node,
    const AnalyticExpansionNodes& expanded_nodes)
{
...
template<>
typename AnalyticExpansion<Node2D>::NodePtr
AnalyticExpansion<Node2D>::tryAnalyticExpansion(const NodePtr&    current_node,
                                                const NodePtr&    goal_node,
                                                const NodeGetter& getter,
                                                int& analytic_iterations,
                                                int& closest_distance)
