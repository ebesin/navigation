    ...
    std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
    ...
    _raw_plan_publisher =
        node->create_publisher<nav_msgs::msg::Path>("unsmoothed_plan", 1);

    RCLCPP_INFO(
        _logger, "Configuring %s of type SmacPlannerLattice", name.c_str());
    ...
    _tolerance = static_cast<float>(
        node->get_parameter(name + ".tolerance").as_double());
    nav2_util::declare_parameter_if_not_declared(
    ...
    node->get_parameter(name + ".max_on_approach_iterations",
    ...
    nav2_util::declare_parameter_if_not_declared(
        ...
        rclcpp::ParameterValue(
            ...
            "output.json"));
    node->get_parameter(name + ".lattice_filepath",
                        _search_info.lattice_filepath);
    nav2_util::declare_parameter_if_not_declared(
        node,
        name + ".cache_obstacle_heuristic",
        rclcpp::ParameterValue(false));
    node->get_parameter(name + ".cache_obstacle_heuristic",
                        _search_info.cache_obstacle_heuristic);
    nav2_util::declare_parameter_if_not_declared(
        node, name + ".reverse_penalty", rclcpp::ParameterValue(2.0));
    node->get_parameter(name + ".reverse_penalty",
    ...
    node->get_parameter(name + ".non_straight_penalty",
    ...
    _metadata =
        LatticeMotionTable::getLatticeMetadata(_search_info.lattice_filepath);
    ...
    float lookup_table_dim = static_cast<float>(_lookup_table_size) /
    ...
    _collision_checker.setFootprint(costmap_ros->getRobotFootprint(),
                                    costmap_ros->getUseRadius(),
                                    findCircumscribedCost(costmap_ros));

    // Initialize A* template
    _a_star = std::make_unique<AStarAlgorithm<NodeLattice>>(_motion_model,
                                                            _search_info);
    ...
    RCLCPP_INFO(_logger,
                ...
                "and %s. Tolerance %.2f. Using motion model: %s. State lattice "
                "file: %s.",
                _name.c_str(),
                _max_iterations,
                _max_on_approach_iterations,
...
void SmacPlannerLattice::activate()
{
    ...
    _dyn_params_handler = node->add_on_set_parameters_callback(
        std::bind(&SmacPlannerLattice::dynamicParametersCallback,
                  this,
                  std::placeholders::_1));
}
    ...
    const geometry_msgs::msg::PoseStamped& goal)
{
    ...
    _a_star->setStart(mx,
                      my,
                      NodeLattice::motion_table.getClosestAngularBin(
    ...
    try {
        if (!_a_star->createPath(
                path,
                num_iterations,
                _tolerance / static_cast<float>(_costmap->getResolution()))) {
    ...
    for (int i = path.size() - 1; i >= 0; --i) {
        ...
        if (fabs(pose.pose.position.x - last_pose.pose.position.x) < 1e-4 &&
            ...
            fabs(tf2::getYaw(pose.pose.orientation) -
                 tf2::getYaw(last_pose.pose.orientation)) < 1e-4) {
            RCLCPP_DEBUG(_logger,
                         "Removed a path from the path due to replication. "
                         "Make sure your minimum control set does not contain "
                         "duplicate values!");
            continue;
        }
        last_pose = pose;
    ...
    std::vector<rclcpp::Parameter> parameters)
{
    ...
    for (auto parameter : parameters) {
        ...
        if (type == ParameterType::PARAMETER_DOUBLE) {
            ...
            if (name == _name + ".lattice_filepath") {
                ...
                _metadata = LatticeMotionTable::getLatticeMetadata(
                    _search_info.lattice_filepath);
                _search_info.minimum_turning_radius =
                    _metadata.min_turning_radius / (_costmap->getResolution());
            }
    ...
    if (reinit_a_star || reinit_smoother) {
        ...
        // Make sure its a whole number
        lookup_table_dim =
            static_cast<float>(static_cast<int>(lookup_table_dim));

        // Make sure its an odd number
        if (static_cast<int>(lookup_table_dim) % 2 == 0) {
            RCLCPP_INFO(_logger,
                        "Even sized heuristic lookup table size set %f, "
                        "increasing size by 1 to make odd",
                        lookup_table_dim);
            lookup_table_dim += 1.0;
        }
...
PLUGINLIB_EXPORT_CLASS(nav2_smac_planner::SmacPlannerLattice,
