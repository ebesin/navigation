...
GoalPoseSubscriber2D::GoalPoseSubscriber2D(std::string sub_topic_name, const rclcpp::NodeOptions& options)
...
nav2_util::CallbackReturn GoalPoseSubscriber2D::on_configure(const rclcpp_lifecycle::State& state)
{
...
nav2_util::CallbackReturn GoalPoseSubscriber2D::on_activate(const rclcpp_lifecycle::State& state)
{
...
nav2_util::CallbackReturn GoalPoseSubscriber2D::on_deactivate(const rclcpp_lifecycle::State& state)
{
...
nav2_util::CallbackReturn GoalPoseSubscriber2D::on_cleanup(const rclcpp_lifecycle::State& state)
{
...
nav2_util::CallbackReturn GoalPoseSubscriber2D::on_shutdown(const rclcpp_lifecycle::State& state)
{
...
void GoalPoseSubscriber2D::messageCallBack(const geometry_msgs::msg::PoseStamped::SharedPtr goal_pose_ptr)
{
    ...
    RCLCPP_INFO_STREAM(get_logger(),
                       "receive_goal_pose------>"
                           << "x: " << goal_pose_ptr->pose.position.x << " y: " << goal_pose_ptr->pose.position.y
