 ...
 * navigation2 中行为树的ActionNode应该都要继承这个类，其实主要的功能就是实现一个antion的订阅和处理，只不过标准化了
 */
    ...
    bool onGoalReceived(typename ActionT::Goal::ConstSharedPtr goal)
    {
        if (plugin_muxer_->isNavigating()) {
            RCLCPP_ERROR(logger_,
                         "Requested navigation from %s while another navigator "
                         "is processing,"
                         " rejecting request.",
                         getName().c_str());
            return false;
