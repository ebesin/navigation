...
bool Smoother::smooth(nav_msgs::msg::Path& path,
  ...
  RCLCPP_INFO_STREAM(rclcpp::get_logger("SmacPlannerSmoother"),
                     "path size: " << path.poses.size());
  ...
  for (unsigned int i = 0; i != path_segments.size(); i++) {
    if (path_segments[i].end - path_segments[i].start > 10) {
      ...
      time_remaining =
          max_time - duration_cast<duration<double>>(now - start).count();

      // Smooth path segment naively
      const geometry_msgs::msg::Pose start_pose =
          curr_path_segment.poses.front().pose;
      const geometry_msgs::msg::Pose goal_pose =
          curr_path_segment.poses.back().pose;
      bool local_success = smoothImpl(curr_path_segment, reversing_segment,
                                      costmap, time_remaining);
      ...
      if (!is_holonomic_ && local_success) {
        enforceStartBoundaryConditions(start_pose, curr_path_segment, costmap,
                                       reversing_segment);
        enforceEndBoundaryConditions(goal_pose, curr_path_segment, costmap,
                                     reversing_segment);
      }
...
bool Smoother::smoothImpl(nav_msgs::msg::Path& path, bool& reversing_segment,
                          const nav2_costmap_2d::Costmap2D* costmap,
                          const double& max_time) {
  ...
  while (change >= tolerance_) {
    ...
    if (timespan > max_dur) {
      RCLCPP_DEBUG(rclcpp::get_logger("SmacPlannerSmoother"),
                   "Smoothing time exceeded allowed duration of %0.2f.",
                   max_time);
      path = last_path;
      updateApproximatePathOrientations(path, reversing_segment);
      return false;
    ...
    for (unsigned int i = 1; i != path_size - 1; i++) {
      ...
      if (costmap) {
        costmap->worldToMap(getFieldByDim(new_path.poses[i], 0),
                            getFieldByDim(new_path.poses[i], 1), mx, my);
...
double Smoother::getFieldByDim(const geometry_msgs::msg::PoseStamped& msg,
...
void Smoother::setFieldByDim(geometry_msgs::msg::PoseStamped& msg,
...
std::vector<PathSegment> Smoother::findDirectionalPathSegments(
  ...
  for (unsigned int idx = 1; idx < path.poses.size() - 1; ++idx) {
    ...
    // vectors.
    double oa_x =
        path.poses[idx].pose.position.x - path.poses[idx - 1].pose.position.x;
    double oa_y =
        path.poses[idx].pose.position.y - path.poses[idx - 1].pose.position.y;
    double ab_x =
        path.poses[idx + 1].pose.position.x - path.poses[idx].pose.position.x;
    double ab_y =
        path.poses[idx + 1].pose.position.y - path.poses[idx].pose.position.y;
...
void Smoother::findBoundaryExpansion(
  ...
  static ompl::base::ScopedState<> from(state_space_), to(state_space_),
      s(state_space_);
  ...
  for (double i = 0; i <= expansion.path_end_idx; i++) {
    ...
    if (static_cast<float>(costmap->getCost(mx, my)) >= 253) {
...
BoundaryExpansions Smoother::generateBoundaryExpansionPoints(IteratorT start,
                                                             IteratorT end) {
...
void Smoother::enforceStartBoundaryConditions(
  ...
  BoundaryExpansions boundary_expansions =
      generateBoundaryExpansionPoints<PathIterator>(path.poses.begin(),
                                                    path.poses.end());
  ...
  for (unsigned int i = 0; i != boundary_expansions.size(); i++) {
    ...
    if (!reversing_segment) {
      findBoundaryExpansion(start_pose, path.poses[expansion.path_end_idx].pose,
                            expansion, costmap);
    } else {
      findBoundaryExpansion(path.poses[expansion.path_end_idx].pose, start_pose,
                            expansion, costmap);
    }
  }

  // Find the shortest kinematically feasible boundary expansion
  unsigned int best_expansion_idx =
      findShortestBoundaryExpansionIdx(boundary_expansions);
...
void Smoother::enforceEndBoundaryConditions(
  ...
  // Find range of points for testing
  BoundaryExpansions boundary_expansions =
      generateBoundaryExpansionPoints<ReversePathIterator>(path.poses.rbegin(),
                                                           path.poses.rend());
  ...
  for (unsigned int i = 0; i != boundary_expansions.size(); i++) {
    ...
    if (!reversing_segment) {
      findBoundaryExpansion(path.poses[expansion_starting_idx].pose, end_pose,
                            expansion, costmap);
    } else {
      findBoundaryExpansion(end_pose, path.poses[expansion_starting_idx].pose,
                            expansion, costmap);
    }
  }
  ...
  for (unsigned int i = 0; i != best_expansion.pts.size(); i++) {
    path.poses[expansion_starting_idx + i].pose.position.x =
        best_expansion.pts[i].x;
    path.poses[expansion_starting_idx + i].pose.position.y =
        best_expansion.pts[i].y;
