...
using mpc_msgs::msg::PathPointWithLaneId;
using mpc_msgs::msg::PathWithLaneId;
using mpc_msgs::msg::Trajectory;
using mpc_msgs::msg::TrajectoryPoint;

namespace motion_utils {
TrajectoryPoint calcInterpolatedPoint(const Trajectory&               trajectory,
                                      ...
                                      const double                    yaw_threshold)
{
    ...
    const auto  v1      = mpc_utils::point2tfVector(curr_pt, next_pt);
    const auto  v2      = mpc_utils::point2tfVector(curr_pt, target_pose);
    if (v1.length2() < 1e-3) {
        return curr_pt;
    }
    ...
    // pose interpolation
    interpolated_point.pose = mpc_utils::calcInterpolatedPose(curr_pt, next_pt, clamped_ratio);
    ...
    if (use_zero_order_hold_for_twist) {
        ...
        interpolated_point.longitudinal_velocity_mps =
            interpolation::lerp(curr_pt.longitudinal_velocity_mps, next_pt.longitudinal_velocity_mps, clamped_ratio);
        interpolated_point.lateral_velocity_mps =
            interpolation::lerp(curr_pt.lateral_velocity_mps, next_pt.lateral_velocity_mps, clamped_ratio);
    ...
    interpolated_point.heading_rate_rps = interpolation::lerp(curr_pt.heading_rate_rps, next_pt.heading_rate_rps, clamped_ratio);

    // wheel interpolation
    interpolated_point.front_wheel_angle_rad =
        interpolation::lerp(curr_pt.front_wheel_angle_rad, next_pt.front_wheel_angle_rad, clamped_ratio);
    ...
    const double interpolated_time = interpolation::lerp(
        rclcpp::Duration(curr_pt.time_from_start).seconds(), rclcpp::Duration(next_pt.time_from_start).seconds(), clamped_ratio);
    interpolated_point.time_from_start = rclcpp::Duration::from_seconds(interpolated_time);

    return interpolated_point;
}

PathPointWithLaneId calcInterpolatedPoint(const PathWithLaneId&           path,
                                          const geometry_msgs::msg::Pose& target_pose,
                                          const bool                      use_zero_order_hold_for_twist,
                                          const double                    dist_threshold,
                                          const double                    yaw_threshold)
{
    ...
    const size_t segment_idx =
        motion_utils::findFirstNearestSegmentIndexWithSoftConstraints(path.points, target_pose, dist_threshold, yaw_threshold);

    // Calculate interpolation ratio
    const auto& curr_pt = path.points.at(segment_idx);
    const auto& next_pt = path.points.at(segment_idx + 1);
    const auto  v1      = mpc_utils::point2tfVector(curr_pt.point, next_pt.point);
    const auto  v2      = mpc_utils::point2tfVector(curr_pt.point, target_pose);
    if (v1.length2() < 1e-3) {
        return curr_pt;
    }
    ...
    // pose interpolation
    interpolated_point.point.pose = mpc_utils::calcInterpolatedPose(curr_pt.point, next_pt.point, clamped_ratio);
    ...
    if (use_zero_order_hold_for_twist) {
        ...
        interpolated_point.point.longitudinal_velocity_mps =
            interpolation::lerp(curr_pt.point.longitudinal_velocity_mps, next_pt.point.longitudinal_velocity_mps, clamped_ratio);
        interpolated_point.point.lateral_velocity_mps =
            interpolation::lerp(curr_pt.point.lateral_velocity_mps, next_pt.point.lateral_velocity_mps, clamped_ratio);
    }
