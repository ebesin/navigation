 ...
 */
template<class T> void validateNonEmpty(const T& points)
{
    if (points.empty()) {
 ...
 */
template<class T>
void validateNonSharpAngle(const T& point1, const T& point2, const T& point3, const double angle_threshold = mpc_utils::pi / 4)
{
    const auto p1 = mpc_utils::getPoint(point1);
    const auto p2 = mpc_utils::getPoint(point2);
    const auto p3 = mpc_utils::getPoint(point3);
    ...
    const auto dist_1to2 = mpc_utils::calcDistance3d(p1, p2);
    const auto dist_3to2 = mpc_utils::calcDistance3d(p3, p2);
 ...
 */
template<class T> boost::optional<bool> isDrivingForward(const T points)
{
    if (points.size() < 2) {
        return boost::none;
    }
    ...
    const auto& first_pose  = mpc_utils::getPose(points.at(0));
    const auto& second_pose = mpc_utils::getPose(points.at(1));

    return mpc_utils::isDrivingForward(first_pose, second_pose);
}
 ...
 */
template<class T> boost::optional<bool> isDrivingForwardWithTwist(const T points_with_twist)
{
    if (points_with_twist.empty()) {
        return boost::none;
    }
    if (points_with_twist.size() == 1) {
        if (0.0 < mpc_utils::getLongitudinalVelocity(points_with_twist.front())) {
            return true;
        }
        else if (0.0 > mpc_utils::getLongitudinalVelocity(points_with_twist.front())) {
            return false;
        }
        else {
            return boost::none;
 ...
 */
template<class T> T removeOverlapPoints(const T& points, const size_t& start_idx = 0)
{
    ...
    for (size_t i = start_idx + 1; i < points.size(); ++i) {
        const auto   prev_p = mpc_utils::getPoint(dst.back());
        const auto   curr_p = mpc_utils::getPoint(points.at(i));
        const double dist   = mpc_utils::calcDistance2d(prev_p, curr_p);
        if (dist < eps) {
            continue;
        }
        dst.push_back(points.at(i));
 ...
 */
template<class T>
boost::optional<size_t> searchZeroVelocityIndex(const T& points_with_twist, const size_t src_idx, const size_t dst_idx)
{
    try {
 ...
 * @return first matching index of a zero velocity point inside the points container.
 */
template<class T> boost::optional<size_t> searchZeroVelocityIndex(const T& points_with_twist, const size_t& src_idx)
{
    try {
        validateNonEmpty(points_with_twist);
    }
 ...
 * @return first matching index of a zero velocity point inside the points container.
 */
template<class T> boost::optional<size_t> searchZeroVelocityIndex(const T& points_with_twist)
{
 ...
 */
template<class T> size_t findNearestIndex(const T& points, const geometry_msgs::msg::Point& point)
{
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        const auto dist = mpc_utils::calcSquaredDistance2d(points.at(i), point);
        if (dist < min_dist) {
            min_dist = dist;
            min_idx  = i;
        }
 ...
 */
template<class T>
boost::optional<size_t> findNearestIndex(const T&                        points,
                                         ...
                                         const double                    max_yaw  = std::numeric_limits<double>::max())
{
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        const auto squared_dist = mpc_utils::calcSquaredDistance2d(points.at(i), pose);
        if (squared_dist > max_squared_dist || squared_dist >= min_squared_dist) {
            continue;
        }

        const auto yaw = mpc_utils::calcYawDeviation(mpc_utils::getPose(points.at(i)), pose);
 ...
 */
template<class T>
double calcLongitudinalOffsetToSegment(const T&                         points,
                                       ...
                                       const bool                       throw_exception = false)
{
    ...
    const auto p_front = mpc_utils::getPoint(overlap_removed_points.at(seg_idx));
    const auto p_back  = mpc_utils::getPoint(overlap_removed_points.at(seg_idx + 1));
 ...
 */
template<class T> size_t findNearestSegmentIndex(const T& points, const geometry_msgs::msg::Point& point)
{
 ...
 */
template<class T>
boost::optional<size_t> findNearestSegmentIndex(const T&                        points,
                                                const geometry_msgs::msg::Pose& pose,
                                                const double                    max_dist = std::numeric_limits<double>::max(),
                                                const double                    max_yaw  = std::numeric_limits<double>::max())
{
 ...
 */
template<class T>
double calcLateralOffset(const T&                         points,
                         const geometry_msgs::msg::Point& p_target,
                         const size_t                     seg_idx,
                         const bool                       throw_exception = false)
{
 ...
 */
template<class T>
double calcLateralOffset(const T& points, const geometry_msgs::msg::Point& p_target, const bool throw_exception = false)
{
    const auto overlap_removed_points = removeOverlapPoints(points, 0);
 ...
 */
template<class T> double calcSignedArcLength(const T& points, const size_t src_idx, const size_t dst_idx)
{
    try {
        validateNonEmpty(points);
    }
    ...
    double dist_sum = 0.0;
    for (size_t i = src_idx; i < dst_idx; ++i) {
        dist_sum += mpc_utils::calcDistance2d(points.at(i), points.at(i + 1));
    }
    return dist_sum;
}
 ...
 */
template<class T> std::vector<double> calcSignedArcLengthPartialSum(const T& points, const size_t src_idx, const size_t dst_idx)
{
    try {
        validateNonEmpty(points);
    }
 ...
 */
template<class T> double calcSignedArcLength(const T& points, const geometry_msgs::msg::Point& src_point, const size_t dst_idx)
{
    try {
        validateNonEmpty(points);
    }
    ...
    const double signed_length_src_offset = calcLongitudinalOffsetToSegment(points, src_seg_idx, src_point);
 ...
 */
template<class T> double calcSignedArcLength(const T& points, const size_t src_idx, const geometry_msgs::msg::Point& dst_point)
{
    try {
        validateNonEmpty(points);
    }
 ...
 */
template<class T>
double calcSignedArcLength(const T&                         points,
                           ...
                           const geometry_msgs::msg::Point& dst_point)
{
    ...
    const double signed_length_src_offset = calcLongitudinalOffsetToSegment(points, src_seg_idx, src_point);
    const double signed_length_dst_offset = calcLongitudinalOffsetToSegment(points, dst_seg_idx, dst_point);
 ...
 */
template<class T> double calcArcLength(const T& points)
{
    try {
        validateNonEmpty(points);
    }
 ...
 */
template<class T> inline std::vector<double> calcCurvature(const T& points)
{
    std::vector<double> curvature_vec(points.size());

    for (size_t i = 1; i < points.size() - 1; ++i) {
        const auto p1       = mpc_utils::getPoint(points.at(i - 1));
        const auto p2       = mpc_utils::getPoint(points.at(i));
        const auto p3       = mpc_utils::getPoint(points.at(i + 1));
        curvature_vec.at(i) = (mpc_utils::calcCurvature(p1, p2, p3));
    }
 ...
 */
template<class T> inline std::vector<std::pair<double, double>> calcCurvatureAndArcLength(const T& points)
{
    ...
    for (size_t i = 1; i < points.size() - 1; ++i) {
        ...
        const auto   p2        = mpc_utils::getPoint(points.at(i));
        const auto   p3        = mpc_utils::getPoint(points.at(i + 1));
        const double curvature = mpc_utils::calcCurvature(p1, p2, p3);
        const double arc_length =
            mpc_utils::calcDistance2d(points.at(i - 1), points.at(i)) + mpc_utils::calcDistance2d(points.at(i), points.at(i + 1));
 ...
 */
template<class T> boost::optional<double> calcDistanceToForwardStopPoint(const T& points_with_twist, const size_t src_idx = 0)
{
    try {
        validateNonEmpty(points_with_twist);
    }
    ...
    const auto closest_stop_idx = searchZeroVelocityIndex(points_with_twist, src_idx, points_with_twist.size());
    if (!closest_stop_idx) {
        return boost::none;
    }
 ...
 */
template<class T>
inline boost::optional<geometry_msgs::msg::Point> calcLongitudinalOffsetPoint(const T&     points,
                                                                              ...
                                                                              const bool   throw_exception = false)
{
    ...
    if (src_idx + 1 == points.size() && offset == 0.0) {
        return mpc_utils::getPoint(points.at(src_idx));
    }

    if (offset < 0.0) {
        auto reverse_points = points;
        std::reverse(reverse_points.begin(), reverse_points.end());
        return calcLongitudinalOffsetPoint(reverse_points, reverse_points.size() - src_idx - 1, -offset);
    }
    ...
    for (size_t i = src_idx; i < points.size() - 1; ++i) {
        ...
        const auto dist_segment = mpc_utils::calcDistance2d(p_front, p_back);
        dist_sum += dist_segment;

        const auto dist_res = offset - dist_sum;
        if (dist_res <= 0.0) {
            return mpc_utils::calcInterpolatedPoint(p_back, p_front, std::abs(dist_res / dist_segment));
        }
    }
 ...
 */
template<class T>
inline boost::optional<geometry_msgs::msg::Pose>
calcLongitudinalOffsetPose(const T&     points,
                           const size_t src_idx,
                           const double offset,
                           ...
                           const bool   throw_exception                         = false)
{
    ...
    if (src_idx + 1 == points.size() && offset == 0.0) {
        return mpc_utils::getPose(points.at(src_idx));
    }
    ...
    if (offset < 0.0) {
        ...
        for (size_t i = reverse_points.size() - src_idx - 1; i < reverse_points.size() - 1; ++i) {
            ...
            const auto dist_res = -offset - dist_sum;
            if (dist_res <= 0.0) {
                return mpc_utils::calcInterpolatedPose(
                    p_back, p_front, std::abs(dist_res / dist_segment), set_orientation_from_position_direction);
            }
        }
    }
    else {
        ...
        for (size_t i = src_idx; i < points.size() - 1; ++i) {
            ...
            if (dist_res <= 0.0) {
                return mpc_utils::calcInterpolatedPose(
                    p_front, p_back, 1.0 - std::abs(dist_res / dist_segment), set_orientation_from_position_direction);
            }
        }
    }
                           ...
                           const bool                       set_orientation_from_position_direction = true)
{
    ...
    return calcLongitudinalOffsetPose(
        points, src_seg_idx, offset + signed_length_src_offset, set_orientation_from_position_direction);
}
 ...
 */
template<class T>
inline boost::optional<size_t> insertTargetPoint(const size_t                     seg_idx,
                                                 const geometry_msgs::msg::Point& p_target,
                                                 T&                               points,
                                                 const double                     overlap_threshold = 1e-3)
{
    ...
    const auto p_front = mpc_utils::getPoint(points.at(seg_idx));
    const auto p_back  = mpc_utils::getPoint(points.at(seg_idx + 1));
    ...
    const auto overlap_with_front = mpc_utils::calcDistance2d(p_target, p_front) < overlap_threshold;
    const auto overlap_with_back  = mpc_utils::calcDistance2d(p_target, p_back) < overlap_threshold;
    ...
    geometry_msgs::msg::Pose target_pose;
    {
        ...
        const auto pitch  = mpc_utils::calcElevationAngle(p_target, p_base);
        const auto yaw    = mpc_utils::calcAzimuthAngle(p_target, p_base);

        target_pose.position    = p_target;
        target_pose.orientation = mpc_utils::createQuaternionFromRPY(0.0, pitch, yaw);
    }

    auto p_insert = points.at(seg_idx);
    mpc_utils::setPose(target_pose, p_insert);

    geometry_msgs::msg::Pose base_pose;
    {
        const auto p_base = is_driving_forward.get() ? p_front : p_back;
        const auto pitch  = mpc_utils::calcElevationAngle(p_base, p_target);
        const auto yaw    = mpc_utils::calcAzimuthAngle(p_base, p_target);

        base_pose.position    = mpc_utils::getPoint(p_base);
        base_pose.orientation = mpc_utils::createQuaternionFromRPY(0.0, pitch, yaw);
    }

    if (!overlap_with_front && !overlap_with_back) {
        if (is_driving_forward.get()) {
            mpc_utils::setPose(base_pose, points.at(seg_idx));
        }
        else {
            mpc_utils::setPose(base_pose, points.at(seg_idx + 1));
        }
 ...
 */
template<class T>
inline boost::optional<size_t> insertTargetPoint(const double                     insert_point_length,
                                                 const geometry_msgs::msg::Point& p_target,
                                                 T&                               points,
                                                 const double                     overlap_threshold = 1e-3)
{
 ...
 */
template<class T>
inline boost::optional<size_t> insertTargetPoint(const size_t src_segment_idx,
                                                 ...
                                                 const double overlap_threshold = 1e-3)
{
    ...
    const double target_length  = insert_point_length - calcSignedArcLength(points, src_segment_idx, *segment_idx);
    const double ratio          = std::clamp(target_length / segment_length, 0.0, 1.0);
    const auto   p_target       = mpc_utils::calcInterpolatedPoint(
        mpc_utils::getPoint(points.at(*segment_idx)), mpc_utils::getPoint(points.at(*segment_idx + 1)), ratio);
 ...
 */
template<class T>
inline boost::optional<size_t> insertTargetPoint(const geometry_msgs::msg::Pose& src_pose,
                                                 const double                    insert_point_length,
                                                 T&                              points,
                                                 const double                    max_dist = std::numeric_limits<double>::max(),
                                                 const double                    max_yaw  = std::numeric_limits<double>::max(),
                                                 const double                    overlap_threshold = 1e-3)
{
    ...
    const double offset_length = calcLongitudinalOffsetToSegment(points, *nearest_segment_idx, src_pose.position);

    return insertTargetPoint(*nearest_segment_idx, insert_point_length + offset_length, points, overlap_threshold);
}
 ...
 */
template<class T>
inline boost::optional<size_t> insertStopPoint(const size_t src_segment_idx,
                                               ...
                                               const double overlap_threshold = 1e-3)
{
    ...
    const auto stop_idx = insertTargetPoint(src_segment_idx, distance_to_stop_point, points_with_twist, overlap_threshold);
    if (!stop_idx) {
        return boost::none;
    }

    for (size_t i = *stop_idx; i < points_with_twist.size(); ++i) {
        mpc_utils::setLongitudinalVelocity(0.0, points_with_twist.at(i));
    }
 ...
 */
template<class T>
inline boost::optional<size_t> insertStopPoint(const double distance_to_stop_point,
                                               T&           points_with_twist,
                                               const double overlap_threshold = 1e-3)
{
    validateNonEmpty(points_with_twist);
    ...
    for (size_t i = 0; i < points_with_twist.size() - 1; ++i) {
        const auto   curr_pose = mpc_utils::getPose(points_with_twist.at(i));
        const auto   next_pose = mpc_utils::getPose(points_with_twist.at(i + 1));
        const double length    = mpc_utils::calcDistance2d(curr_pose, next_pose);
 ...
 */
template<class T>
inline boost::optional<size_t> insertStopPoint(const geometry_msgs::msg::Pose& src_pose,
                                               const double                    distance_to_stop_point,
                                               T&                              points_with_twist,
                                               const double                    max_dist = std::numeric_limits<double>::max(),
                                               const double                    max_yaw  = std::numeric_limits<double>::max(),
                                               const double                    overlap_threshold = 1e-3)
{
    ...
    const auto stop_idx =
        insertTargetPoint(src_pose, distance_to_stop_point, points_with_twist, max_dist, max_yaw, overlap_threshold);
 ...
 */
template<class T>
boost::optional<size_t> insertStopPoint(const size_t                     stop_seg_idx,
                                        const geometry_msgs::msg::Point& stop_point,
                                        T&                               points_with_twist,
                                        const double                     overlap_threshold = 1e-3)
{
    const auto insert_idx = motion_utils::insertTargetPoint(stop_seg_idx, stop_point, points_with_twist, overlap_threshold);
 ...
 */
template<class T>
boost::optional<size_t> insertDecelPoint(const geometry_msgs::msg::Point& src_point,
                                         const double                     distance_to_decel_point,
                                         const double                     velocity,
                                         T&                               points_with_twist)
{
    const auto decel_point = calcLongitudinalOffsetPoint(points_with_twist, src_point, distance_to_decel_point);
    ...
    for (size_t i = insert_idx.get(); i < points_with_twist.size(); ++i) {
        const auto& original_velocity = mpc_utils::getLongitudinalVelocity(points_with_twist.at(i));
        mpc_utils::setLongitudinalVelocity(std::min(original_velocity, velocity), points_with_twist.at(i));
    }
 ...
 */
template<class T> void insertOrientation(T& points, const bool is_driving_forward)
{
    if (is_driving_forward) {
        for (size_t i = 0; i < points.size() - 1; ++i) {
            const auto&  src_point = mpc_utils::getPoint(points.at(i));
            const auto&  dst_point = mpc_utils::getPoint(points.at(i + 1));
            const double pitch     = mpc_utils::calcElevationAngle(src_point, dst_point);
            const double yaw       = mpc_utils::calcAzimuthAngle(src_point, dst_point);
            mpc_utils::setOrientation(mpc_utils::createQuaternionFromRPY(0.0, pitch, yaw), points.at(i));
            if (i == points.size() - 2) {
                // Terminal orientation is same as the point before it
                mpc_utils::setOrientation(mpc_utils::getPose(points.at(i)).orientation, points.at(i + 1));
            }
        }
    }
    else {
        ...
        mpc_utils::setOrientation(mpc_utils::getPose(points.at(1)).orientation, points.at(0));
    }
}
 ...
 */
template<class T> void removeInvalidOrientationPoints(T& points, const double max_yaw_diff = M_PI_2)
{
    for (size_t i = 1; i < points.size();) {
        const double yaw1 = tf2::getYaw(mpc_utils::getPose(points.at(i - 1)).orientation);
        const double yaw2 = tf2::getYaw(mpc_utils::getPose(points.at(i)).orientation);

        if (max_yaw_diff < std::abs(mpc_utils::normalizeRadian(yaw1 - yaw2))) {
 ...
 */
template<class T>
size_t findFirstNearestIndexWithSoftConstraints(const T&                        points,
                                                const geometry_msgs::msg::Pose& pose,
                                                ...
                                                const double yaw_threshold  = std::numeric_limits<double>::max())
{
    ...
    {   // with dist and yaw thresholds
        ...
        for (size_t i = 0; i < points.size(); ++i) {
            const auto squared_dist = mpc_utils::calcSquaredDistance2d(points.at(i), pose.position);
            const auto yaw          = mpc_utils::calcYawDeviation(mpc_utils::getPose(points.at(i)), pose);
 ...
 */
template<class T>
size_t findFirstNearestSegmentIndexWithSoftConstraints(const T&                        points,
                                                       const geometry_msgs::msg::Pose& pose,
                                                       const double dist_threshold = std::numeric_limits<double>::max(),
                                                       const double yaw_threshold  = std::numeric_limits<double>::max())
{
    // find first nearest index with soft constraints (not segment index)
    const size_t nearest_idx = findFirstNearestIndexWithSoftConstraints(points, pose, dist_threshold, yaw_threshold);
 ...
 */
template<class T>
boost::optional<double> calcDistanceToForwardStopPoint(const T&                        points_with_twist,
                                                       const geometry_msgs::msg::Pose& pose,
                                                       const double max_dist = std::numeric_limits<double>::max(),
                                                       const double max_yaw  = std::numeric_limits<double>::max())
{
    ...
    const auto nearest_segment_idx = motion_utils::findNearestSegmentIndex(points_with_twist, pose, max_dist, max_yaw);
    ...
    const auto stop_idx =
        motion_utils::searchZeroVelocityIndex(points_with_twist, *nearest_segment_idx + 1, points_with_twist.size());

    if (!stop_idx) {
        return boost::none;
    }

    const auto closest_stop_dist = calcSignedArcLength(points_with_twist, pose.position, *nearest_segment_idx, *stop_idx);
...
template<typename T>
T cropForwardPoints(const T&                         points,
                    const geometry_msgs::msg::Point& target_pos,
                    const size_t                     target_seg_idx,
                    const double                     forward_length)
{
    ...
    double sum_length = -motion_utils::calcLongitudinalOffsetToSegment(points, target_seg_idx, target_pos);
    for (size_t i = target_seg_idx + 1; i < points.size(); ++i) {
        sum_length += mpc_utils::calcDistance2d(points.at(i), points.at(i - 1));
...
template<typename T>
T cropBackwardPoints(const T&                         points,
                     const geometry_msgs::msg::Point& target_pos,
                     const size_t                     target_seg_idx,
             ...
             const double                     backward_length)
{
    ...
    const auto cropped_forward_points = cropForwardPoints(points, target_pos, target_seg_idx, forward_length);

    const size_t modified_target_seg_idx = std::min(target_seg_idx, cropped_forward_points.size() - 2);
    const auto cropped_points = cropBackwardPoints(cropped_forward_points, target_pos, modified_target_seg_idx, backward_length);

    if (cropped_points.size() < 2) {
        RCLCPP_ERROR(rclcpp::get_logger("obstacle_avoidance_planner.trajectory_utils"),
 ...
 */
template<class T>
double calcYawDeviation(const T& points, const geometry_msgs::msg::Pose& pose, const bool throw_exception = false)
{
    const auto overlap_removed_points = removeOverlapPoints(points, 0);
    ...
    const double path_yaw = mpc_utils::calcAzimuthAngle(mpc_utils::getPoint(overlap_removed_points.at(seg_idx)),
                                                        mpc_utils::getPoint(overlap_removed_points.at(seg_idx + 1)));
    const double pose_yaw = tf2::getYaw(pose.orientation);

    return mpc_utils::normalizeRadian(pose_yaw - path_yaw);
}
 ...
 */
template<class T>
bool isTargetPointFront(const T&                         points,
                        const geometry_msgs::msg::Point& base_point,
