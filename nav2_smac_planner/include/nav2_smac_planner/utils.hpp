    ...
    const float& mx, const float& my, const nav2_costmap_2d::Costmap2D* costmap)
{
    ...
    msg.position.x = static_cast<float>(costmap->getOriginX()) +
                     (mx + 0.5) * costmap->getResolution();
    msg.position.y = static_cast<float>(costmap->getOriginY()) +
    ...
    std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap)
{
    ...
    if (!inflation_layer_found) {
        RCLCPP_WARN(rclcpp::get_logger("computeCircumscribedCost"),
                    ...
                    "If this is an SE2-collision checking plugin, it cannot "
                    "use costmap potential "
