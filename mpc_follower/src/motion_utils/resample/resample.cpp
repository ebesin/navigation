...
std::vector<geometry_msgs::msg::Point> resamplePointVector(const std::vector<geometry_msgs::msg::Point>& points,
                                                           ...
                                                           const bool                                    use_akima_spline_for_xy,
                                                           const bool                                    use_lerp_for_z)
{
    ...
    for (size_t i = 1; i < points.size(); ++i) {
        ...
        const double ds      = mpc_utils::calcDistance2d(prev_pt, curr_pt);
    ...
    // Interpolate
    const auto lerp   = [&](const auto& input) { return interpolation::lerp(input_arclength, input, resampled_arclength); };
    const auto spline = [&](const auto& input) { return interpolation::spline(input_arclength, input, resampled_arclength); };
                                                         ...
                                                         const bool                                   use_lerp_for_z)
{
    ...
    const auto resampled_position = resamplePointVector(position, resampled_arclength, use_akima_spline_for_xy, use_lerp_for_z);
    ...
    const bool is_driving_forward = mpc_utils::isDrivingForward(points.at(0), points.at(1));
                                           ...
                                           const bool                           use_zero_order_hold_for_v)
{
    ...
    for (size_t i = 0; i < input_path.points.size(); ++i) {
        ...
        for (size_t j = 1; j < resampling_arclength.size(); ++j) {
            if (resampling_arclength.at(j - 1) <= distance_to_resampling_point &&
                distance_to_resampling_point < resampling_arclength.at(j)) {
                const double dist_to_prev_point      = std::fabs(distance_to_resampling_point - resampling_arclength.at(j - 1));
                const double dist_to_following_point = std::fabs(resampling_arclength.at(j) - distance_to_resampling_point);
                if (dist_to_prev_point < motion_utils::overlap_threshold) {
                    ...
                    resampling_arclength.insert(resampling_arclength.begin() + j, distance_to_resampling_point);
                }
                break;
            }
        }
    ...
    for (size_t i = 1; i < input_path.points.size(); ++i) {
        ...
        const double ds      = mpc_utils::calcDistance2d(prev_pt.pose.position, curr_pt.pose.position);
        input_arclength.push_back(ds + input_arclength.back());
    ...
    if (input_arclength.back() < resampling_arclength.back()) {
        std::cerr << "[motion_utils]: resampled path length is longer than input path length" << std::endl;
        return input_path;
    }

    // Interpolate
    const auto lerp = [&](const auto& input) { return interpolation::lerp(input_arclength, input, resampling_arclength); };

    auto closest_segment_indices = interpolation::calc_closest_segment_indices(input_arclength, resampling_arclength);
    ...
    for (size_t i = 0; i < resampling_arclength.size(); ++i) {
        ...
        if (std::abs(input_arclength.at(seg_idx) - resampling_arclength.at(i)) <= epsilon) {
            interpolated_lane_ids.at(i).insert(interpolated_lane_ids.at(i).end(), prev_lane_ids.begin(), prev_lane_ids.end());
        }
        else if (std::abs(input_arclength.at(seg_idx + 1) - resampling_arclength.at(i)) <= epsilon) {
            interpolated_lane_ids.at(i).insert(interpolated_lane_ids.at(i).end(), next_lane_ids.begin(), next_lane_ids.end());
        }
        else {
            // extract lane_ids those prev_lane_ids and next_lane_ids have in common
            for (const auto target_lane_id : prev_lane_ids) {
                if (std::find(next_lane_ids.begin(), next_lane_ids.end(), target_lane_id) != next_lane_ids.end()) {
    ...
    if (interpolated_pose.size() != resampling_arclength.size()) {
        std::cerr << "[motion_utils]: Resampled pose size is different from resampled arclength" << std::endl;
        return input_path;
    }

    mpc_msgs::msg::PathWithLaneId resampled_path;
    ...
    for (size_t i = 0; i < resampled_path.points.size(); ++i) {
        mpc_msgs::msg::PathPoint path_point;
                                           ...
                                           const bool                           resample_input_path_stop_point)
{
    ...
    std::vector<mpc_msgs::msg::PathPoint> transformed_input_path(input_path.points.size());
    for (size_t i = 0; i < input_path.points.size(); ++i) {
    ...
    if (resample_input_path_stop_point) {
        const auto distance_to_stop_point = motion_utils::calcDistanceToForwardStopPoint(transformed_input_path, 0);
        if (distance_to_stop_point && !resampling_arclength.empty()) {
            for (size_t i = 1; i < resampling_arclength.size(); ++i) {
                if (resampling_arclength.at(i - 1) <= *distance_to_stop_point &&
                    *distance_to_stop_point < resampling_arclength.at(i)) {
                    const double dist_to_prev_point      = std::fabs(*distance_to_stop_point - resampling_arclength.at(i - 1));
                    const double dist_to_following_point = std::fabs(resampling_arclength.at(i) - *distance_to_stop_point);
                    if (dist_to_prev_point < motion_utils::overlap_threshold) {
    ...
    return resamplePath(input_path, resampling_arclength, use_akima_spline_for_xy, use_lerp_for_z, use_zero_order_hold_for_v);
}
                                 ...
                                 const bool                 use_zero_order_hold_for_v)
{
    ...
    mpc_msgs::msg::Path resampled_path;
    resampled_path.header      = input_path.header;
    resampled_path.left_bound  = input_path.left_bound;
                                 ...
                                 const bool                 resample_input_path_stop_point)
{
    ...
    // Insert stop point
    if (resample_input_path_stop_point) {
        const auto distance_to_stop_point = motion_utils::calcDistanceToForwardStopPoint(input_path.points, 0);
        if (distance_to_stop_point && !resampling_arclength.empty()) {
            for (size_t i = 1; i < resampling_arclength.size(); ++i) {
                if (resampling_arclength.at(i - 1) <= *distance_to_stop_point &&
                    *distance_to_stop_point < resampling_arclength.at(i)) {
    ...
    return resamplePath(input_path, resampling_arclength, use_akima_spline_for_xy, use_lerp_for_z, use_zero_order_hold_for_twist);
}
                                             ...
                                             const bool                       use_zero_order_hold_for_twist)
{
    ...
    time_from_start.push_back(rclcpp::Duration(input_trajectory.points.front().time_from_start).seconds());
    ...
    const auto interpolated_heading_rate      = lerp(heading_rate);
    const auto interpolated_acceleration      = use_zero_order_hold_for_twist ? zoh(acceleration) : lerp(acceleration);
    ...
    mpc_msgs::msg::Trajectory resampled_trajectory;
    ...
    for (size_t i = 0; i < resampled_trajectory.points.size(); ++i) {
        mpc_msgs::msg::TrajectoryPoint traj_point;
...
mpc_msgs::msg::Trajectory resampleTrajectory(const mpc_msgs::msg::Trajectory& input_trajectory,
                                             ...
                                             const bool                       use_lerp_for_z,
                                             const bool                       use_zero_order_hold_for_twist,
                                             const bool                       resample_input_trajectory_stop_point)
{
    // validate arguments
    ...
    if (resample_input_trajectory_stop_point) {
        const auto distance_to_stop_point = motion_utils::calcDistanceToForwardStopPoint(input_trajectory.points, 0);
        if (distance_to_stop_point && !resampling_arclength.empty()) {
            for (size_t i = 1; i < resampling_arclength.size(); ++i) {
                if (resampling_arclength.at(i - 1) <= *distance_to_stop_point &&
                    *distance_to_stop_point < resampling_arclength.at(i)) {
    ...
    return resampleTrajectory(
        input_trajectory, resampling_arclength, use_akima_spline_for_xy, use_lerp_for_z, use_zero_order_hold_for_twist);
}
