...
public:
   ...
   */
    BT::NodeStatus tick() override
    {
                ...
                if (goal_updated_ &&
                    ...
                    if (!is_future_goal_handle_complete(elapsed)) {
                        ...
                        RCLCPP_WARN(node_->get_logger(),
                                    ...
                                    "to acknowledge goal request for %s",
                                    action_name_.c_str());
                        future_goal_handle_.reset();
                        return BT::NodeStatus::FAILURE;
                    }
        ...
        switch (result_.code) {
        case rclcpp_action::ResultCode::SUCCEEDED: status = on_success(); break;

        case rclcpp_action::ResultCode::ABORTED: status = on_aborted(); break;
