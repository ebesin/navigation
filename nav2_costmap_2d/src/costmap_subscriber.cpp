    ...
    : topic_name_(topic_name)
{
    ...
    costmap_sub_ = node->create_subscription<nav2_msgs::msg::Costmap>(
        ...
        std::bind(
            &CostmapSubscriber::costmapCallback, this, std::placeholders::_1));
}

CostmapSubscriber::CostmapSubscriber(const rclcpp::Node::WeakPtr& parent,
                                     const std::string&           topic_name)
    : topic_name_(topic_name)
{
    auto node    = parent.lock();
...
void CostmapSubscriber::toCostmap2D()
{
    ...
    if (costmap_ == nullptr) {
        costmap_ = std::make_shared<Costmap2D>(
            current_costmap_msg->metadata.size_x,
            current_costmap_msg->metadata.size_y,
    ...
    else if (costmap_->getSizeInCellsX() !=
                 current_costmap_msg->metadata.size_x ||   // NOLINT
        ...
        costmap_->resizeMap(current_costmap_msg->metadata.size_x,
                            current_costmap_msg->metadata.size_y,
                            current_costmap_msg->metadata.resolution,
                            current_costmap_msg->metadata.origin.position.x,
                            current_costmap_msg->metadata.origin.position.y);
    ...
    for (unsigned int i = 0; i < current_costmap_msg->metadata.size_x; ++i) {
        for (unsigned int j = 0; j < current_costmap_msg->metadata.size_y;
             ++j) {
